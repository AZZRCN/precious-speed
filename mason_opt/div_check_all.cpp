// div_check_all.cpp - 对拍驱动: 跑多组测试用例
// 编译: g++ -O2 -o div_check_all.exe div_check_all.cpp
// 运行: div_check_all.exe
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main() {
    ...
    Case cases[] = {
        ...
        {"a_eq_b_nonzero", 5,    100,    100,   57},  // 注意: a 可能 == b
    };
    int n = sizeof(cases) / sizeof(cases[0]);
    int pass = 0, fail = 0;
    for (int i = 0; i < n; i++) {
        char cmd[512];
        std::snprintf(cmd, sizeof(cmd), "div_check.exe %d %d %d %u %s",
