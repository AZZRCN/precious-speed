...
echo "=== compile DEC div.cpp ==="
g++ -O3 -std=c++17 -march=znver3 -mtune=znver3 -o dec_best/origin/dec_div_zn.bin dec_best/origin/div.cpp 2>&1 | tail -5
echo "DEC_BUILD_RC=$?"
echo "=== compile HEX div.cpp ==="
g++ -O3 -std=c++17 -march=znver3 -mtune=znver3 -o hex_best/div_zn.bin hex_best/div.cpp 2>&1 | tail -5
echo "HEX_BUILD_RC=$?"
...
for g in length_ratio_0 length_ratio_1 length_ratio_2 length_ratio_3 length_ratio_4 length_ratio_5 amax_0 amax_1 amax_2 max_0 max_1 max_2 bzbound_0 bzbound_1 bzbound_2 bzbound_3 large_0 large_1 power_0 rnear_1 rnear_2 medium_0 small_0; do
  ...
  echo "$g $dv $hv" >> /tmp/cmp.tsv
done
echo "=== CMP TSV (group DEC_inst HEX_inst) ==="
cat /tmp/cmp.tsv
