src = open('../hex_best/mul.cpp', encoding='utf-8').read().splitlines()
stop = next(i for i,l in enumerate(src) if '#ifdef V16_SELFTEST' in l)
head = src[:stop]
main = r'''
#include <cstdio>
#include <cstring>
...
'''
open('mulgtest_mul.cpp','w',encoding='utf-8').write('\n'.join(head)+main)
print("wrote mulgtest_mul.cpp lines:", stop+main.count('\n'))
