...
#ifndef FFT_LEAF_LOG
// LEAF=11 == #391969(17ms) 历史最优配置。此前 Intel VM 误判 13 更优,
// 但 AMD LC 实测 #392236(LEAF=13)=18ms 反慢 1ms —— 以 LC(AMD) 为准, 维持 11。
...
static int hex2limbs(const char* s, int nhex, u64* out){
    ...
    for(int c=0;c<nc;++c){
        int lo = nhex - (c+1)*16; int start = lo<0?0:lo;
        int len = nhex - 16*c - start;
        u64 v=0; for(int k=0;k<len;++k) v=(v<<4)|nib(s[start+k]);
        out[c]=v;
    }
...
static void mulg(const u64* a, int na, const u64* b, int nb, u64* c){
    if (nb <= 48) mul_bf(a, na, b, nb, c); else mul_fft(a, na, b, nb, c);
}
int main(){
    std::string A, B; std::getline(std::cin, A); std::getline(std::cin, B);
