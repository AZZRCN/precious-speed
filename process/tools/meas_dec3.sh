...
echo "=== compile DEC div.cpp (c++20 -O2, faithful to LC) ==="
g++ -O2 -std=c++20 -o dec_best/origin/dec_div_zn.bin dec_best/origin/div.cpp 2>&1 | tail -8
echo "DEC_BUILD_DONE rc=$?"

# correctness spot-check on small_0
./dec_best/origin/dec_div_zn.bin < cases_dec/small_0.in > /tmp/dec_small0.out 2>/dev/null
echo "DEC_SMALL0_OUT_DONE lines=$(wc -l < /tmp/dec_small0.out) rc=$?"
...
echo "=== DEC instructions per group (c++20 -O2) ==="
cat /tmp/cmp_dec.tsv
