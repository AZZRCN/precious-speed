...
int main(){
    static char buf[1<<20]; long r=read(0,buf,sizeof buf-1); if(r<=0) return 1; buf[r]=0;
    const char* p=buf;
    while(*p){
        while(*p==' '||*p=='\n'||*p=='\r')++p;
        if(!*p) break;
        const char* d0=p; while(*p && *p!='\n' && *p!='\r')++p;
        int nd=(int)(p-d0);
        int n=hex2limbs(d0,nd,WORK); while(n>1&&WORK[n-1]==0)--n;
        int sigma=(int)_lzcnt_u64(WORK[n-1]);
        u64* dn = WORK + 100000;
        if(sigma){ unsigned char c=0; for(int i=0;i<n;++i){ u64 cur=WORK[i]; dn[i]=(cur<<sigma)|c; c=cur>>(64-sigma);} }
        else std::memcpy(dn,WORK,(size_t)n*8);
        wp = WORK + 200000;
        u64* v = WORK + 500000;
        invertappr(dn, n, v);
