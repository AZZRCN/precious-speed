...
int main(){
    ...
    int nd = hex2limbs(D.c_str(),(int)D.size(),WORK);
    while(nd>1&&WORK[nd-1]==0)--nd;
    u64* v = WORK+2000000;
    invertappr(WORK, nd, v);
    char buf[4000000]; limbs2hex(v, nd, buf); printf("%s\n", buf);
    return 0;
}
'''
open('invtest.cpp','w',encoding='utf-8').write('\n'.join(head)+main)
print("wrote invtest.cpp")
