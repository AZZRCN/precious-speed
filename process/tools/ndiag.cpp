...
static int hex2limbs(const char* s, int nhex, u64* out){
    int nc=(nhex+15)>>4; auto nib=[](char c)->int{if(c>='0'&&c<='9')return c-'0';if(c>='a'&&c<='f')return c-'a'+10;if(c>='A'&&c<='F')return c-'A'+10;return 0;};
...
static void limbs2hex(const u64* V,int n,char* out){while(n>1&&V[n-1]==0)--n;for(int i=n-1;i>=0;--i){for(int b=60;b>=0;b-=4){int d=(V[i]>>b)&0xF;*out++=d<10?('0'+d):('A'+d-10);}}*out=0;}
static void pr(const char* t,const u64* V,int n){char o[4000000];limbs2hex(V,n,o);printf("%s=%s\n",t,o);}
int main(){
    char buf[1<<20]; long r=read(0,buf,sizeof buf-1); if(r<=0)return 1; buf[r]=0;
    const char* p=buf; while(*p==' '||*p=='\n'||*p=='\r')++p; const char* a0=p; while(*p&&*p!='\n'&&*p!=' '&&*p!='\r')++p; int la=(int)(p-a0);
    while(*p==' ')++p; const char* b0=p; while(*p&&*p!='\n'&&*p!='\r')++p; int lb=(int)(p-b0);
    int na=hex2limbs(a0,la,A); int nb=hex2limbs(b0,lb,B);
    while(na>1&&A[na-1]==0)--na; while(nb>1&&B[nb-1]==0)--nb;
    if(mag_cmp(A,na,B,nb)<0){printf("SKIP a<b\n");return 0;}
    int n=nb;
    u64* Qo=WORK+0; u64* Ro=WORK+(na-nb+8);
    wp=WORK+2*na+64;
    // ---- replicate newton_divide with prints ----
    const int sigma=(int)_lzcnt_u64(B[n-1]);
    u64* dn=wp; wp+=n+1; u64* an=wp; wp+=na+1; u64* v=wp; wp+=n+1;
    if(sigma){unsigned char c=0;for(int i=0;i<n;++i){u64 cur=B[i];dn[i]=(cur<<sigma)|c;c=cur>>(64-sigma);}c=0;for(int i=0;i<na;++i){u64 cur=A[i];an[i]=(cur<<sigma)|c;c=cur>>(64-sigma);}an[na]=c;}
    else{std::memcpy(dn,B,(size_t)n*8);std::memcpy(an,A,(size_t)na*8);an[na]=0;}
    const int na_an=na+1;
    invertappr(dn,n,v);
    u64* L=wp; wp+=na_an+n+1; mulg(an,na_an,v,n,L);
    const int qn=na-n+1;
    u64* qe=wp; wp+=qn+1; std::memset(qe,0,(size_t)(qn+1)*8);
    { unsigned char c=0; for(int i=0;i<qn;++i){u64 x=(n+i<na_an)?an[n+i]:0; u64 y=(2*n+i<na_an+n)?L[2*n+i]:0; u64 s; c=_addcarry_u64(c,x,y,(unsigned long long*)&s); qe[i]=s;} qe[qn]=c; }
    pr("qest_pre",qe,qn+1);
    { unsigned char c=0; for(int i=0;i<2*n;++i){u64 x=(i>=n)?an[i-n]:0; u64 s; c=_addcarry_u64(c,x,L[i],(unsigned long long*)&s);} if(c){unsigned char cc=1;for(int i=0;i<qn+1&&cc;++i){u64 cur=qe[i];qe[i]=cur+cc;cc=(cur+cc<cur);}} }
    pr("qest_post",qe,qn+1);
    u64* QD=wp; wp+=na_an+1; std::memset(QD,0,(size_t)(na_an+1)*8); mulg(qe,qn,dn,n,QD);
    u64* Rt=wp; wp+=na_an+1;
    { unsigned char b=0; for(int i=0;i<na_an;++i){u64 sd=QD[i];u64 s;b=_subborrow_u64(b,an[i],sd,(unsigned long long*)&s);Rt[i]=s;} if(b){printf("QEST_OVERFLOW b=1\n");unsigned char cc=1;for(int i=0;i<qn&&cc;++i){u64 cur=qe[i];qe[i]=cur-cc;cc=(cur<cc);}unsigned char ad=0;for(int i=0;i<na_an;++i){u64 sd=(i<n)?dn[i]:0;u64 s;ad=_addcarry_u64(ad,Rt[i],sd,(unsigned long long*)&s);Rt[i]=s;}} }
    // consistency: recompute an2 = qe*dn + Rt
    u64* QD2=wp; wp+=na_an+1; std::memset(QD2,0,(size_t)(na_an+1)*8); mulg(qe,qn,dn,n,QD2);
    u64* an2=wp; wp+=na_an+1; std::memset(an2,0,(size_t)(na_an)*8);
    { unsigned char br=0; for(int i=0;i<na_an;++i){u64 s;br=_addcarry_u64(br,Rt[i],QD2[i],(unsigned long long*)&s);an2[i]=s;} if(br){an2[na_an]=1;} }
    int match=1; for(int i=0;i<na_an;++i) if(an2[i]!=an[i]) match=0;
    printf("CONSIST_an==qe*dn+Rt: %s\n", match?"YES":"NO");
    // correction loop
    for(int iter=0;iter<32;++iter){
        bool ge=false; for(int i=na_an-1;i>=n;--i) if(Rt[i]){ge=true;break;}
        if(!ge){int cmp=0;for(int i=n-1;i>=0;--i){if(Rt[i]!=dn[i]){cmp=(Rt[i]>dn[i])?1:-1;break;}}ge=(cmp>=0);}
        if(!ge)break;
        unsigned char cc=1; for(int i=0;i<qn;++i){u64 s;cc=_addcarry_u64(cc,qe[i],0,(unsigned long long*)&s);qe[i]=s;if(!cc)break;}
        unsigned char sb=0; for(int i=0;i<na_an;++i){u64 sd=(i<n)?dn[i]:0;u64 s;sb=_subborrow_u64(sb,Rt[i],sd,(unsigned long long*)&s);Rt[i]=s;}
    }
    { unsigned char br=0; for(int i=0;i<na_an;++i){u64 s;br=_addcarry_u64(br,Rt[i],QD2[i],(unsigned long long*)&s);an2[i]=s;} if(br)an2[na_an]=1; }
    match=1; for(int i=0;i<na_an;++i) if(an2[i]!=an[i]) match=0;
    printf("CONSIST_after_corr: %s  Rt<dn? ", match?"YES":"NO");
    { bool ge=false; for(int i=na_an-1;i>=n;--i)if(Rt[i]){ge=true;break;} if(!ge){int cmp=0;for(int i=n-1;i>=0;--i){if(Rt[i]!=dn[i]){cmp=(Rt[i]>dn[i])?1:-1;break;}}ge=(cmp>=0);} printf("%s\n",ge?"NO":"YES"); }
    pr("FINAL_q",qe,qn); pr("FINAL_Rt",Rt,na_an);
    // extract q,r
    std::memcpy(Qo,qe,(size_t)qn*8);
    if(sigma){for(int i=0;i<n;++i){u64 lo=Rt[i]>>sigma;u64 hi=(i+1<na_an)?(Rt[i+1]<<(64-sigma)):0;Ro[i]=lo|hi;}} else std::memcpy(Ro,Rt,(size_t)n*8);
    pr("Q",Qo,qn); pr("R",Ro,n);
    return 0;
}
