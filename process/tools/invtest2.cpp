...
static constexpr size_t HP = 1u << 7;   // 2 MiB 透明大页
...
static constexpr int MAXC = 4000;          // 1.6M hex / 16
#ifndef BZ_CUTOFF
...
alignas(HP) static u64 WORK[300000];                    // Newton/BZ 工作栈 (~32MB), 峰值约 16*n
static u64* wp = WORK;

static constexpr u32 LMMAX = 1u << 15;
...
static constexpr u32 FMCAP = 1u << 15;
...
int main(){
    ...
    const char* p=buf; while(*p==' '||*p=='\n')++p; const char* d0=p; while(*p!='\n'&&*p)++p; int nd=(int)(p-d0);
    int n=hex2limbs(d0,nd,WORK); while(n>1&&WORK[n-1]==0)--n;
    wp = WORK + 2*n + 32;                 // scratch clear of d (at WORK[0..n))
    u64* v = WORK + 250000;               // safe offset
    invertappr(WORK, n, v);
    char o[4000000]; limbs2hex(v, n, o); printf("%s\n", o);
    return 0;
}
