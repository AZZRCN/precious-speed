...
// #26b(A/B): 单级大表。twflat[i] = cmul(base_lo[i&mask], base_hi[i>>halfLog]) 预乘好,
//   twg/twlo 退化为一次 load (省 1 load + 1 复乘)。值逐位等同两级表 -> 输出 bytecmp 相同。
//   实测(tw_idx): 最大索引恒为 n/2-1, 故只需 n/2 项。ts<=2^22 -> 2^21 项 = 32 MiB。
//   代价: 每次 resize 需 O(n/2) 建表 (生产每 mul_fft 仅 1 次 resize + 3 次变换)。
#define TW_FLAT_MAX (1u << 21)
alignas(64) static __m128d twflat[TW_FLAT_MAX];
...
// ---- resize 调用剖析 (仅 rsprof 构建) ----
struct RsProf {
    u32 seq[8192]; u32 ns = 0;      // 前 8192 次真实重建的 n 序列
    unsigned long long rebuilds = 0, calls = 0, entries = 0;
    ~RsProf() {
        fprintf(stderr, "[rsprof] calls=%llu rebuilds=%llu entries_built=%llu\n",
                calls, rebuilds, entries);
        u32 uniq[64]; unsigned long long cnt[64] = {0}; u32 nu = 0;
        for (u32 i = 0; i < ns; ++i) {
            u32 j = 0; for (; j < nu; ++j) if (uniq[j] == seq[i]) break;
            if (j == nu) { if (nu >= 64) continue; uniq[nu] = seq[i]; cnt[nu] = 0; ++nu; }
            ++cnt[j];
        }
        fprintf(stderr, "[rsprof] distinct=%u :", nu);
        for (u32 j = 0; j < nu; ++j) fprintf(stderr, " %u x%llu", uniq[j], cnt[j]);
        fprintf(stderr, "\n[rsprof] first40:");
        for (u32 i = 0; i < ns && i < 40; ++i) fprintf(stderr, " %u", seq[i]);
        fprintf(stderr, "\n");
    }
};
static RsProf g_rsprof;
static void resize(u32 n) {  // n = 复数点数; 只建 16 KiB 的 base 表
    ++g_rsprof.calls;
    if (n == twN) return;
    ++g_rsprof.rebuilds; g_rsprof.entries += (n >> 1);
    if (g_rsprof.ns < 8192) g_rsprof.seq[g_rsprof.ns++] = n;
    twN = n;
    ...
    const u32 cap = n >> 1;
    for (u32 i = 0; i != cap; ++i)
        twflat[i] = cmul(twbase[i & twHalfMask], twbase[twHalfSize | (i >> twHalfLog)]);
}
// 单级查表: 1 次 load, 无复乘
static inline cpx twg(u32 i) { return twflat[i]; }
// 大表下高位外提无意义: twlo 直接查表 (忽略 hi), twhi 返回占位
static inline cpx twlo(u32 i, cpx) { return twflat[i]; }
