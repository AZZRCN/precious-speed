...
// PROFILE 前向声明 (定义见下方 CYCB 区)
static void rec_mul(unsigned int len, bool cyc);
#endif
...
// ===== PROFILE 插桩: 乘法尺寸直方图 (monster 乘法次数/长度分布) =====
#include <map>
struct MulRec { long long cnt = 0; long long sumlen = 0; };
static std::map<u32, MulRec> g_mulhist;   // 线性 mul_fft
static std::map<u32, MulRec> g_cychist;   // 环形 cyc_mul_*
static void rec_mul(u32 len, bool cyc) {
    auto& m = cyc ? g_cychist : g_mulhist;
    m[len].cnt++; m[len].sumlen += (long long)len;
}
static void dump_mulhist() {
    fprintf(stderr, "\n==== MUL HISTOGRAM (len=FFT real points) ====\n");
    fprintf(stderr, "-- linear mul_fft --\n");
    for (auto& kv : g_mulhist)
        fprintf(stderr, "  L=%-9u calls=%-7lld  sumL=%-12lld  proxyIr~=%-14.0f\n",
                kv.first, kv.second.cnt, kv.second.sumlen,
                (double)kv.second.cnt * kv.first * (64 - __builtin_clz(kv.first)));
    fprintf(stderr, "-- cyclic cyc_mul_* --\n");
    for (auto& kv : g_cychist)
        fprintf(stderr, "  L=%-9u calls=%-7lld  sumL=%-12lld  proxyIr~=%-14.0f\n",
                kv.first, kv.second.cnt, kv.second.sumlen,
                (double)kv.second.cnt * kv.first * (64 - __builtin_clz(kv.first)));
    double tot = 0; long long tc = 0;
    for (auto& kv : g_mulhist) { tot += (double)kv.second.cnt*kv.first*(64-__builtin_clz(kv.first)); tc+=kv.second.cnt; }
    for (auto& kv : g_cychist)  { tot += (double)kv.second.cnt*kv.first*(64-__builtin_clz(kv.first)); tc+=kv.second.cnt; }
    fprintf(stderr, "  TOTAL calls=%-7lld  proxyIr~=%-14.0f\n", tc, tot);
}
