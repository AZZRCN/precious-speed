#!/usr/bin/env python3
"""Patch cur_div.cpp to add BENCH_DIV_GROUPS mode for grouped stage timing.
Groups cases by len2 (b length) to identify which path is slow."""
import sys
...
def patch_cur_div(src_path, out_path):
    ...
    # Find the HINT_OP_DIV main function's non-BENCH_DIV_PURE branch
    old_loop = '''#else
    ...
    new_loop = '''#else
#ifdef BENCH_DIV_GROUPS
    // Group cases by len2 to identify which path is slow
    ...
    double t_div_cmp_eq = 0, t_div_cmp_lt = 0;
    size_t cnt_cmp_eq = 0, cnt_cmp_lt = 0;
    hint::Integer a, b, q, r;
    while (t--)
    {
        ...
        int cmp_pre = (a.length() < blen) ? -1 : 0; // pre-check
        a.absDivRem(b, q, r);
        ...
        *oCursor++ = '\\n';
        ...
        // Group by len2
        if (blen == 0) { cnt_cmp_eq++; } // b=0 case
        else if (blen == 1) { t_div_len2_1 += std::chrono::duration<double, std::milli>(_s2 - _s1).count(); cnt_len2_1++; }
...
if __name__ == '__main__':
    patch_cur_div('cur_div.cpp', 'cur_div_groups.cpp')
