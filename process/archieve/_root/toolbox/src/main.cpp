...
// 子命令分发: split | sam | regex | glob | es | tree | head | lines | hex | wc | outline
#include "common.h"
...
static void print_usage() {
    printf(
        ...
        "  hex     十六进制查看器\n"
        "  wc      统计行数/单词数/字节数\n"
        "  outline C++ 代码大纲解析 (类/函数/命名空间, 行号, JSON)\n"
        "\n"
        "GLOBAL OPTIONS:\n"
        "  -h, --help  显示帮助\n"
        "  -json       JSON 输出 (split/sam/regex/glob/es/outline 支持)\n"
        "\n"
...
int main(int argc, char** argv) {
    ...
    if (cmd == "outline") return tbx::cmd_outline(argc, argv);
