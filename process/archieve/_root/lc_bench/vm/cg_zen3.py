#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""callgrind + Zen3 cache 模拟 —— 夜间模式下最贴近 LC 的性能代理。

为什么需要它:
  纯 Ir (指令数) 不是 LC 时间的可靠代理。实测反例: a_max_b_random_02 的
  Ir 全场最低 (291M) 却在 LC 上 68ms 接近最慢 —— 因为它的 Fc=262144 是
  全场最大 FFT, 工作集远超 L2, 访存主导。

  callgrind 的 cache 模拟是纯软件的确定性模拟, 与 CPU 争抢/降频完全无关,
  且可以手动配置成 **LC 判题机 (AMD Zen3)** 的 cache 层次, 而不是本地 VM
  (i7-11370H, L3 12MB) 的。这消除了 memory 里记录的"VM L3 12MB thrash
  假象不可外推 LC"这一老问题。

Zen3 cache: L1d 32KB/8-way/64B, LL(L3) 32MB/16-way/64B
代价模型 (cycles 估计): Ir + 5*D1mr + 200*DLmr  (L2 命中 ~12cyc, DRAM ~200cyc)

用法:
    python cg_zen3.py <bin> [case ...]
    python cg_zen3.py poll
"""
import os
import sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from vmctl import put, run  # noqa: E402

HERE = os.path.dirname(os.path.abspath(__file__))
LOG = "/home/azzr/divbench/logs/cg_zen3.log"

if len(sys.argv) > 1 and sys.argv[1] == "poll":
    rc, out, err = run(f"cat {LOG}; echo '--- alive:'; pgrep -fa cg_zen3.py | head -2")
    print(out)
    sys.exit(0)

BIN = sys.argv[1] if len(sys.argv) > 1 else "d15"
CASES = sys.argv[2:] or [
    "length_ratio_integer_00", "length_ratio_integer_01",
    "length_ratio_integer_02", "length_ratio_integer_03",
    "a_max_b_random_00", "a_max_b_random_01", "a_max_b_random_02",
    "max_00", "large_00", "large_01", "r_nearly_zero_00", "r_nearly_zero_01",
]

REMOTE = '''#!/usr/bin/env python3
import subprocess, os, re
IN = '/home/azzr/lcp/big_integer/division_of_big_integers/in/'
BIN = %r
CASES = %r

# AMD Zen3 (LC judge): L1d 32KB/8way/64B, LL 32MB/16way/64B
CACHE = '--cache-sim=yes --D1=32768,8,64 --I1=32768,8,64 --LL=33554432,16,64'

def measure(c):
    outf = '/tmp/cgz_%%s.out' %% c
    cmd = ('valgrind --tool=callgrind ' + CACHE +
           ' --callgrind-out-file=' + outf + ' ./bin/' + BIN +
           ' < ' + IN + c + '.in > /dev/null 2>/tmp/cgz_%%s.err' %% c)
    if subprocess.run(cmd, shell=True, cwd='/home/azzr/divbench').returncode != 0:
        return None
    # callgrind_annotate 汇总更稳: 直接读 summary/totals 行
    ev, tot = None, None
    with open(outf) as f:
        for line in f:
            if line.startswith('events:'):
                ev = line.split()[1:]
            elif line.startswith('summary:') or line.startswith('totals:'):
                tot = [int(x) for x in line.split()[1:]]
    if not ev or not tot:
        return None
    d = dict(zip(ev, tot))
    return d

hdr = '%%-26s %%13s %%11s %%10s %%13s %%7s' %% (
    'case', 'Ir', 'D1mr+w', 'DLmr+w', 'est_cycles', 'CPI')
print(hdr, flush=True)
print('-' * len(hdr), flush=True)
rows = []
for c in CASES:
    if not os.path.exists(IN + c + '.in'):
        continue
    d = measure(c)
    if not d:
        print('%%-26s FAIL' %% c, flush=True); continue
    Ir = d.get('Ir', 0)
    d1 = d.get('D1mr', 0) + d.get('D1mw', 0)
    dl = d.get('DLmr', 0) + d.get('DLmw', 0)
    est = Ir + 5 * d1 + 200 * dl
    rows.append((c, Ir, d1, dl, est))
    print('%%-26s %%13d %%11d %%10d %%13d %%7.3f' %% (c, Ir, d1, dl, est, est / Ir),
          flush=True)
print('-' * len(hdr), flush=True)
rows.sort(key=lambda r: -r[4])
print('=== ranked by est_cycles (LC bottleneck proxy) ===', flush=True)
for i, r in enumerate(rows[:8]):
    print('  %%d. %%-26s est=%%d  (Ir=%%d, DLm=%%d)' %% (i + 1, r[0], r[4], r[1], r[3]),
          flush=True)
print('DONE', flush=True)
''' % (BIN, CASES)

p = os.path.join(HERE, "_cg_zen3.py")
with open(p, "w", encoding="utf-8", newline="\n") as f:
    f.write(REMOTE)
put(p, "/home/azzr/cg_zen3.py")
run("mkdir -p /home/azzr/divbench/logs && rm -f " + LOG)
run(f"cd /home/azzr && nohup python3 cg_zen3.py > {LOG} 2>&1 &  echo started", timeout=30)
print(f"started cg_zen3 on {BIN}; poll with: python cg_zen3.py poll")
