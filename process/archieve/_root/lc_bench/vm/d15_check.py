...
put(os.path.join(PS, "best", "div_D15.cpp"), "/home/azzr/divbench/src/d15.cpp")
rc, out, err = run(f"cd ~/divbench/src && "
                   f"g++ {F} -o ../bin/d15 d15.cpp || echo BUILDFAIL_d15; "
                   f"g++ {F} -DDIV_MUSHAPE -o ../bin/d15s d15.cpp || echo BUILDFAIL_d15s; "
...
for c in cases:
    ...
    if hs['d15'] != hs['d13']: bad.append(c)
print('== d15 vs d13 (26 cases):', bad if bad else 'ALL MATCH')
EOF'''
...
for c in length_ratio_integer_00 length_ratio_integer_02 length_ratio_integer_03 \
  ...
  ./bin/d15s < $IN/$c.in >/dev/null 2>/tmp/s15.txt
  echo "=== $c"
...
with open(os.path.join(PS, "lc_bench", "vm", "_d15_check.log"), "w", encoding="utf-8") as f:
    f.write(o1 + "\n" + o2)
