# -*- coding: utf-8 -*-
"""D29 = D27(大页 arena) + 融合 submul_1 的 schoolbook 长除法。

原 absDivBasicCore 每个商位要走 3~4 趟 len2:
    absMul1(divisor, qhat, tprod)      写 tprod   (len2+1)
    absCompare(tprod, dividend_span)   读 两边     (通常提前退出)
    absSub(tprod, divisor, tprod)      修正       (偶发)
    absSub(dividend, tprod, dividend)  读 tprod + 读写 dividend

GMP 用融合 submul_1 只走 1 趟: dividend -= divisor*qhat, 借出再 add-back。
本补丁把 D19 的 SWAR 乘法进位链 与 D11 的 SWAR 减法借位链 串到同一个
16-limb 循环体里, tprod 彻底消失。

用 -DDIV_FUSED_SUBMUL=0 可退回原实现做 A/B。
"""
import io, os, sys

SRC = os.path.join(os.path.dirname(__file__), '..', 'best', 'div_D27.cpp')
DST = os.path.join(os.path.dirname(__file__), '..', 'best', 'div_D29.cpp')
...
PRIM = r'''        // === D29: 融合 submul_1 —— np[0..n] -= dp[0..n-1] * x, 单趟 ===
        //
        // 乘法侧沿用 D19 的分解:  p[i] = dp[i]*x = q[i]*BASE + r[i],
        //   t[i] = r[i] + q[i-1] < 2*BASE, 至多 1 的涟漪进位用 SWAR 一次算完 16 limb,
        //   得到真正的乘积 limb m[i]。
        // 减法侧紧接着复用 D11 的 SWAR 借位链: ts = np + BASE - m,
        //   G = (ts < BASE) 生成, P = (ts == BASE) 传播。
        // 两条链在同一个寄存器块里首尾相接, 中间量 m 从不落内存 —— tprod 消失。
        //
        // 返回 true 表示整窗借出 (qhat 估大了), 调用方 add-back 并 qhat--。
        static bool absSubMul1(Limb *np, const Limb *dp, size_t n, Limb x)
        {
            ...
            if (n >= 16)
            {
                ...
                for (; i + 15 < n; i += 16)
                {
                    ...
                    // t 现在就是乘积 limb m, 直接进减法链, 不落内存
                    // ---- 减法侧: np[i..i+15] -= m
                    __m256i d = _mm256_loadu_si256(
                        reinterpret_cast<const __m256i *>(np + i));
                    __m256i ts = _mm256_sub_epi16(_mm256_add_epi16(d, vbase16), t);
                    ...
                    _mm256_storeu_si256(reinterpret_cast<__m256i *>(np + i), res);
                }
            }
            // ---- 标量尾部: 乘法进位与减法借位继续串
...
print('[B] absSubMul1 / absAddBackDiv inserted')
...
print('[C] absDivBasicCore main loop fused')
