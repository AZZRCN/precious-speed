...
namespace mulprof {
    ...
    struct Scope { int i; std::chrono::high_resolution_clock::time_point t0;
    ...
    void dump(){ double tot=0; for(int i=0;i<24;i++) if(recs[i].name) tot+=recs[i].ms;
        fprintf(stderr,"MULPROF_TOTAL %.3f
",tot); fflush(stderr);
        for(int i=0;i<24;i++) if(recs[i].name && recs[i].n) fprintf(stderr,"MULPROF %-12s %8.3f ms (%lld calls)
",recs[i].name,recs[i].ms,(long long)recs[i].n); fflush(stderr); }
}
