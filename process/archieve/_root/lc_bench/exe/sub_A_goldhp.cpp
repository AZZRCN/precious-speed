...
//
// 候选 A（本文件）取 22：2^22 > 任何可能的 transformSize，顶层判定一次即落入
// 扁平循环，递归分支永不进入 —— 运行期等价于原榜首的广度优先 radix-2，
// 与 gold 的唯一差异只剩内存分配走 2 MB 大页。这样即使 Zen3 上缓存分块
// 不转移（甚至有害），也不会亏；THP 若被判题机关掉则自动回退到 gold 原路径。
#ifndef MUL_FFT_LEAF_LOG
#define MUL_FFT_LEAF_LOG 22
#endif
