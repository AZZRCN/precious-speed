...
> **注**：本文档为历史开发记录，引用的 `moptm_fusion.cpp` 已归档到 `archieve/cpp/moptm_fusion.cpp`。引用的 `scripts/` 目录已从 git tracking 移除（本地保留）。`fuzz_*.py`、`benchmark*.py` 等脚本不再 tracked。
> **后续开发记录**请追加到本文件末尾，不要删除历史记录。
