// =============================================================================
// supopt_safe.cpp – 绝对安全版 (Zen 3)
// 所有 Barrett 除法使用"余数检查"修正（安全优先）
// 与 supopt_fast.cpp 仅 divBASE32/divBASE64 实现不同
// =============================================================================
#include <immintrin.h>
#include <cstdint>
#include <cstring>
...
constexpr uint32_t BARRETT_M32 = 429497;  // ceil(2^32 / 10000) = 429497

#define HINT_PREFETCH(addr, rw, loc) __builtin_prefetch((addr), (rw), (loc))

// 安全版 Barrett: 检查余数 r 是否 >= BASE (处理 q 偏大 1)
...
// =============================================================================
// 任务 #1: absSub_avx2 直接 store (与 fast 版一致)
// =============================================================================
...
// =============================================================================
// 任务 #2: absMul1 SIMD (与 fast 版一致, divBASE32 自动用安全版)
// =============================================================================
...
// =============================================================================
// 任务 #3: absDiv1 SIMD (用 if 单次修正, 不用 while)
// =============================================================================
static Limb absDiv1_optimized(View in, Limb x, Span out) {
    ...
    while (i >= 8) {
        ...
        for (int k = 7; k >= 0; --k) {
            ...
            if ((uint64_t)q * x > prod) q--;  // 安全版: 仍用单次 if
...
// =============================================================================
// 任务 #4: fftMul carry chain (与 fast 版一致, divBASE64 自动用安全版)
// =============================================================================
