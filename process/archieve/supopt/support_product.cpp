...
constexpr uint64_t BARRETT_M64 = 0x68DB8BAC710CCULL;  // ceil(2^64 / 10000)
constexpr uint32_t BARRETT_M32 = 429496;               // ceil(2^32 / 10000)
...
// =============================================================================
// 任务 #2: absMul1 SIMD 向量化 (16-limb 并行 + 32 位 Barrett)
// =============================================================================
static Limb absMul1_optimized(View in, Limb x, Span out) {
    ...
    for (; i + 15 < in.size; i += 16) {
        // 加载 16 个 limb（两个 128 位向量），扩展为 32 位
        ...
        // 串行 carry 传播（16 次，使用快速 32 位 Barrett）
        for (int k = 0; k < 16; ++k) {
...
// =============================================================================
// 任务 #3: absDiv1 SIMD 优化 (SIMD 搬运 + 64 位 Barrett)
// =============================================================================
...
// =============================================================================
// 任务 #4: fftMul carry chain 向量化 (cvttpd + 64 位 Barrett)
// =============================================================================
static void fftMul_carry_optimized(const double *v1, size_t conv_len, Span out) {
    ...
    for (; i + 7 < conv_len; i += 8) {
        ...
        for (int k = 0; k < 8; ++k) {
            uint64_t s = carry + (uint64_t)vals[k];
            uint64_t q;
...
// =============================================================================
// 任务 #5: absDivBasicCore qhat 优化 (ceil 乘数 + 调用优化子函数)
// =============================================================================
...
static void absDivBasicCore_optimized(Span dividend, View divisor, Span quotient) {
    ...
    while (quot_idx > 0) {
        ...
        if (high1 >= divisor_high) {
            ...
            if (q >= BASE) q = BASE - 1;
            while ((uint64_t)q * divisor_high > high) q--;
            qhat = (Limb)q;
        }
...
//安全补丁：
// 版本二：绝对安全版（supopt_safe.cpp）

// 安全版仅在速度版基础上修改了 Barrett 除法的校正方式（通过检查余数是否大于等于 BASE 来修正），并在任务 #5 中添加了断言。以下仅列出差异部分，完整代码请参照速度版替换对应函数即可。

// 修改的 Barrett 函数：
// cpp

// 32 位 Barrett（安全版）
static inline Limb divBASE32_safe(uint64_t s, uint64_t &q) {
    q = (s * BARRETT_M32) >> 32;
    uint64_t r = s - q * BASE;
    if (r >= BASE) { r += BASE; q--; }
    return Limb(r);
...
// 64 位 Barrett（安全版）
static inline uint64_t divBASE64_safe(uint64_t s, uint64_t &q) {
    q = (uint64_t)((unsigned __int128)s * BARRETT_M64 >> 64);
    uint64_t r = s - q * BASE;
    if (r >= BASE) { r += BASE; q--; }
    return r;
...
// 任务 #5 增加断言（在 qhat = (Limb)q; 之后）：
// cpp

qhat = (Limb)q;
assert(qhat <= BASE - 1);

// 其余所有函数与速度版一致，只需将 divBASE32/divBASE64 替换为对应的 _safe 版本。完整文件只需在速度版基础上做上述替换和添加即可。