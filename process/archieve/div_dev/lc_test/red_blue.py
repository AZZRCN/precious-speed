"""
Red-Blue adversarial test for div_modular.cpp.
Red team: generate test cases targeting specific vulnerabilities.
Blue team: cur_mod.exe.
...
TMP_IN = Path(r"d:\precious_speed\div_dev\lc_test\rb.in")
TMP_OUT = Path(r"d:\precious_speed\div_dev\lc_test\rb.out")

BASE = 10000  # moptm BASE = 10^4


def run_batch(cases):
    """Run cur_mod on batch. Returns list of (q_str, r_str) or None on failure."""
...
def check(case_idx, a, b, mq, mr):
    """Check if cur_mod output matches Python divmod."""
    q, r = divmod(a, b)
    eq, er = str(q), str(r)
    if mq == eq and mr == er:
        return True, None
...
def gen_red_cases():
    ...
    # cyclic_m < divisor_float_len 时启用cyclic
    # 构造divisor长度刚好在cyclic切换边界
    for b_len in [3, 4, 5, 6, 7, 8, 9, 10, 15, 20, 30, 40, 50]:
        b = (BASE ** b_len) // 2 + 1  # 高位=5000, 归一化
        q = (BASE ** (2 * b_len)) - 1
        r = b - 1
        a = q * b + r
    ...
    for b_len in [64, 128, 256]:
        b = (BASE ** b_len) - 1  # 全 9999 除数
        q = (BASE ** (2 * b_len)) - 1  # 全 9999 商
        r = b - 1
        a = q * b + r
...
def main():
    cases = gen_red_cases()
    print(f"Generated {len(cases)} red-team cases. Running blue-team (cur_mod)...", flush=True)
