...
int main() {
    ...
    for (auto c : cases) {
        ...
        std::string sbig = make_number(c.digits * 2 + 1, 3);

        Int a(sa), b(sb), big(sbig);

        std::cout << "[" << c.tag << "] digits=" << c.digits << "\n";
        std::cout << "  add  " << ms([&]{ Int r = a + b; escape(r); }) << " ms\n";
        std::cout << "  sub  " << ms([&]{ Int r = a - (b <= a ? b : a); escape(r); }) << " ms\n";
        std::cout << "  mul  " << ms([&]{ Int r = a * b; escape(r); }) << " ms\n";
        std::cout << "  div  " << ms([&]{ Int r = big / a; escape(r); }) << " ms\n";
        std::cout << std::flush;
    }
