#!/usr/bin/env python3
"""D8 正确性验证: 官方 26 例逐字节哈希 + 对黄金实现 div_orig 的压力测试。

与 d7_verify.py 同构, 仅指向 div_D8 (代价模型选 mu_in, 含倒数 FFT bucket)。
"""
import random
import re
import shutil
...
NAME = sys.argv[1] if len(sys.argv) > 1 else 'div_D8'
...
def official():
    ...
    up = out.upper()
    # 严格判据: 必须出现 "BAD=0" 且不得出现 WA/MISMATCH/FAIL
    ...
    for g, n in GENS.items():
        ...
        for seed in range(1, n + 1):
            ...
            if r1 != 0 or r2 != 0 or a != b:
                ...
                try:
                    shutil.move(inf, f'/home/azzr/divbench/logs/bad_d8_{g}_{seed}.in')
    ...
    for l2, qn in shapes:
        ...
        if len(pairs) >= 12:
            ...
            if r1 != 0 or r2 != 0 or a != b:
                ...
                try:
                    shutil.move(inf, f'/home/azzr/divbench/logs/bad_t8d_{batch}.in')
                except Exception as e:
                    print(f'     (save failed: {e})', flush=True)
            else:
                Path(inf).unlink(missing_ok=True)
