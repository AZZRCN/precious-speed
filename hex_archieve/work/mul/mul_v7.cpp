...
// ============================ 状态 (未提交 LC) ============================
// 实验 v7 = v6 (两级 twiddle 表) + DIV 的 twlo/twhi/mulI 快速路径
//   (difFlat/ditFlat 外提高位分量, 每蝶形 3 cmul -> 2 cmul, 抵消 v6 的 I refs 膨胀)。
// 动机: MUL 是 DRAM 受限 (FB/GB 工作集 ~16MiB 打穿 L3), LLd misses 才是权威指标,
//       v6 已证 LLd misses -15%, 但 I refs +6.5% 被误判回归; v7 把 I refs 也压回。
//   正确性: 待 VM 22 官方用例闸门 + cachegrind 判优。
//   判据: 以 Zen3 LLd misses 为主, I refs 为辅 (墙钟在 LC 抖动, 不可靠)。
// =================================================================
// HEX multiplication v7 (two-level twiddle table + DIV fast path)
...
static void ditFlat(cpx* d, u32 n, u32 bb) {
    ...
    for (; (q << 2) <= n; q <<= 2, bcOut >>= 2) {
        ...
        // base/base2 天然对齐 bcOut/2bcOut; 仅当整块不跨 halfSize 边界才可外提高位分量
