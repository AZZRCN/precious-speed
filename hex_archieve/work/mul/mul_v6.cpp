...
// ============================ LC 回执 ============================
// Submission #391746  2026/8/10 14:40:40  C++23  AC  38 ms  33.06 MiB
//   >>> 未夺榜。榜一更快。<<<
//   计分点 max_max_06 = 38ms ; 同尺寸兄弟点 26~28ms (极差 46% = 带宽抖动特征)
//   本地预测 maxIr 131.3M / maxCyc 74.9M ~ 25.2ms  ->  实测 38ms (偏差 +51%)
//   >>> 教训: Ir 权威在 mul 上失效。差的 51% 全是访存停顿。
//   >>> 33 MiB 工作集 = 2^20 复点 x 16B x 2 buf, 每层蝶形都在打 DRAM。
//   >>> v1..v5 五个版本只砍指令没砍字节 -> 下阶段主攻 DRAM 流量, 不是指令数。
//   逐点明细: archive/submissions/391746_mul_v5_38ms_AC.md
// =================================================================
// HEX multiplication v6 (two-level twiddle table, like div v9)
...
static void difFlat(cpx* d, u32 n, u32 bb) {
    ...
    for (; bs >= 2; bc <<= 2, st = bs >> 1, bs >>= 2) {
        ...
        for (; j != bc; ++j, s += st)
            bf2Fwd(s, q, twg(base + j), twg(base2 + 2 * j), mulI(twg(base2 + 2 * j)));
    }
    if (bs == 1) {  // 层数为奇数时剩最低一层
...
static void ditFlat(cpx* d, u32 n, u32 bb) {
    ...
    for (; (q << 2) <= n; q <<= 2, bcOut >>= 2) {
        ...
        for (; j != bcOut; ++j, s += st)
            bf2Inv(s, q, twg(base + j), twg(base2 + 2 * j), mulI(twg(base2 + 2 * j)));
    }
...
static void difRec(cpx* d, u32 n, u32 bb) {
    ...
    else bf2Fwd(d, q, twg(bb), twg(bb << 1), mulI(twg(bb << 1)));
    difRec(d, q, b4);
    difRec(d + q, q, b4 | 1);
    difRec(d + 2 * q, q, b4 | 2);
    difRec(d + 3 * q, q, b4 | 3);
...
static void ditRec(cpx* d, u32 n, u32 bb) {
    ...
    else bf2Inv(d, q, twg(bb), twg(bb << 1), mulI(twg(bb << 1)));
}
