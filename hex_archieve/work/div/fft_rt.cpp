...
// ---- radix-2^3: 三层融合 radix-8 DIF 蝶形 (8 点融合) ----
// 约定 (与已验证的 radix-4 bf2Fwd 一致, DIF = 先加减后乘 twiddle):
//   stage1 (stride 4q): 合并 k 与 k+4, twiddle W8^k -> u1*w1, u2*w2, u3*w3 (u0 无 twiddle)
//   stage2 (stride 2q): 合并 (t0,t2)/(t1,t3), 仅奇位差乘 W4^1=W8^2=w2 -> v3,r3 (v2,r2 无 twiddle)
//   stage3 (stride q):  W2^0=1, 无 twiddle
static inline void bf8Fwd(cpx* s, u32 q, cpx w1, cpx w2, cpx w3) {
    const __m256d W1 = BC4(w1), W1s = _mm256_permute_pd(W1, 0x5);
    const __m256d W2 = BC4(w2), W2s = _mm256_permute_pd(W2, 0x5);
    const __m256d W3 = BC4(w3), W3s = _mm256_permute_pd(W3, 0x5);
    cpx* e = s + q; cpx* p = s;
    for (; p + 2 <= e; p += 2) {
        __m256d x0 = _mm256_loadu_pd((const double*)p);
        __m256d x1 = _mm256_loadu_pd((const double*)(p + q));
        __m256d x2 = _mm256_loadu_pd((const double*)(p + 2 * q));
        __m256d x3 = _mm256_loadu_pd((const double*)(p + 3 * q));
        __m256d x4 = _mm256_loadu_pd((const double*)(p + 4 * q));
        __m256d x5 = _mm256_loadu_pd((const double*)(p + 5 * q));
        __m256d x6 = _mm256_loadu_pd((const double*)(p + 6 * q));
        __m256d x7 = _mm256_loadu_pd((const double*)(p + 7 * q));
        __m256d s0 = _mm256_add_pd(x0, x4), d0 = _mm256_sub_pd(x0, x4);
        __m256d s1 = _mm256_add_pd(x1, x5), d1 = cmul4(_mm256_sub_pd(x1, x5), W1, W1s);
        __m256d s2 = _mm256_add_pd(x2, x6), d2 = cmul4(_mm256_sub_pd(x2, x6), W2, W2s);
        __m256d s3 = _mm256_add_pd(x3, x7), d3 = cmul4(_mm256_sub_pd(x3, x7), W3, W3s);
        __m256d t0 = _mm256_add_pd(s0, s2), t2 = _mm256_sub_pd(s0, s2);
        __m256d t1 = _mm256_add_pd(s1, s3), t3 = cmul4(_mm256_sub_pd(s1, s3), W2, W2s);
        __m256d u0 = _mm256_add_pd(d0, d2), u2 = _mm256_sub_pd(d0, d2);
        __m256d u1 = _mm256_add_pd(d1, d3), u3 = cmul4(_mm256_sub_pd(d1, d3), W2, W2s);
        __m256d a0 = _mm256_add_pd(t0, t1), a1 = _mm256_sub_pd(t0, t1);
        __m256d a2 = _mm256_add_pd(t2, t3), a3 = _mm256_sub_pd(t2, t3);
        __m256d a4 = _mm256_add_pd(u0, u1), a5 = _mm256_sub_pd(u0, u1);
        __m256d a6 = _mm256_add_pd(u2, u3), a7 = _mm256_sub_pd(u2, u3);
        _mm256_storeu_pd((double*)p, a0); _mm256_storeu_pd((double*)(p + q), a1);
        _mm256_storeu_pd((double*)(p + 2 * q), a2); _mm256_storeu_pd((double*)(p + 3 * q), a3);
        _mm256_storeu_pd((double*)(p + 4 * q), a4); _mm256_storeu_pd((double*)(p + 5 * q), a5);
        _mm256_storeu_pd((double*)(p + 6 * q), a6); _mm256_storeu_pd((double*)(p + 7 * q), a7);
    }
    if (p != e) {
        cpx x0 = p[0], x1 = p[q], x2 = p[2 * q], x3 = p[3 * q], x4 = p[4 * q], x5 = p[5 * q], x6 = p[6 * q], x7 = p[7 * q];
        cpx s0 = _mm_add_pd(x0, x4), d0 = _mm_sub_pd(x0, x4);
        cpx s1 = _mm_add_pd(x1, x5), d1 = cmul(_mm_sub_pd(x1, x5), w1);
        cpx s2 = _mm_add_pd(x2, x6), d2 = cmul(_mm_sub_pd(x2, x6), w2);
        cpx s3 = _mm_add_pd(x3, x7), d3 = cmul(_mm_sub_pd(x3, x7), w3);
        cpx t0 = _mm_add_pd(s0, s2), t2 = _mm_sub_pd(s0, s2);
        cpx t1 = _mm_add_pd(s1, s3), t3 = cmul(_mm_sub_pd(s1, s3), w2);
        cpx u0 = _mm_add_pd(d0, d2), u2 = _mm_sub_pd(d0, d2);
        cpx u1 = _mm_add_pd(d1, d3), u3 = cmul(_mm_sub_pd(d1, d3), w2);
        cpx a0 = _mm_add_pd(t0, t1), a1 = _mm_sub_pd(t0, t1);
        cpx a2 = _mm_add_pd(t2, t3), a3 = _mm_sub_pd(t2, t3);
        cpx a4 = _mm_add_pd(u0, u1), a5 = _mm_sub_pd(u0, u1);
        cpx a6 = _mm_add_pd(u2, u3), a7 = _mm_sub_pd(u2, u3);
        p[0] = a0; p[q] = a1; p[2 * q] = a2; p[3 * q] = a3;
        p[4 * q] = a4; p[5 * q] = a5; p[6 * q] = a6; p[7 * q] = a7;
    }
}
// radix-8 DIT 逆蝶形 = bf8Fwd 的转置+共轭 (stage 顺序反转, twiddle 取 conj):
//   stageA (stride q): 相邻对, 无 twiddle
//   stageB (stride 2q): 还原 stage2, 仅奇位差乘 conj(w2)
//   stageC (stride 4q): 还原 stage1, d1*conj(w1), d2*conj(w2), d3*conj(w3)
static inline void bf8Inv(cpx* s, u32 q, cpx w1, cpx w2, cpx w3) {
    const __m256d W1 = BC4(w1), W1l = _mm256_unpacklo_pd(W1, W1), W1h = _mm256_unpackhi_pd(W1, W1);
    const __m256d W2 = BC4(w2), W2l = _mm256_unpacklo_pd(W2, W2), W2h = _mm256_unpackhi_pd(W2, W2);
    const __m256d W3 = BC4(w3), W3l = _mm256_unpacklo_pd(W3, W3), W3h = _mm256_unpackhi_pd(W3, W3);
    cpx* e = s + q; cpx* p = s;
    for (; p + 2 <= e; p += 2) {
        __m256d a0 = _mm256_loadu_pd((const double*)p);
        __m256d a1 = _mm256_loadu_pd((const double*)(p + q));
        __m256d a2 = _mm256_loadu_pd((const double*)(p + 2 * q));
        __m256d a3 = _mm256_loadu_pd((const double*)(p + 3 * q));
        __m256d a4 = _mm256_loadu_pd((const double*)(p + 4 * q));
        __m256d a5 = _mm256_loadu_pd((const double*)(p + 5 * q));
        __m256d a6 = _mm256_loadu_pd((const double*)(p + 6 * q));
        __m256d a7 = _mm256_loadu_pd((const double*)(p + 7 * q));
        __m256d v0 = _mm256_add_pd(a0, a1), v1 = _mm256_sub_pd(a0, a1);
        __m256d v2 = _mm256_add_pd(a2, a3), v3 = _mm256_sub_pd(a2, a3);
        __m256d r0 = _mm256_add_pd(a4, a5), r1 = _mm256_sub_pd(a4, a5);
        __m256d r2 = _mm256_add_pd(a6, a7), r3 = _mm256_sub_pd(a6, a7);
        __m256d s0 = _mm256_add_pd(v0, v2), s2 = _mm256_sub_pd(v0, v2);
        __m256d s1 = _mm256_add_pd(v1, cmulconj4(v3, W2l, W2h)), s3 = _mm256_sub_pd(v1, cmulconj4(v3, W2l, W2h));
        __m256d d0 = _mm256_add_pd(r0, r2), d2 = _mm256_sub_pd(r0, r2);
        __m256d d1 = _mm256_add_pd(r1, cmulconj4(r3, W2l, W2h)), d3 = _mm256_sub_pd(r1, cmulconj4(r3, W2l, W2h));
        __m256d x0 = _mm256_add_pd(s0, d0), x4 = _mm256_sub_pd(s0, d0);
        __m256d x1 = _mm256_add_pd(s1, cmulconj4(d1, W1l, W1h)), x5 = _mm256_sub_pd(s1, cmulconj4(d1, W1l, W1h));
        __m256d x2 = _mm256_add_pd(s2, cmulconj4(d2, W2l, W2h)), x6 = _mm256_sub_pd(s2, cmulconj4(d2, W2l, W2h));
        __m256d x3 = _mm256_add_pd(s3, cmulconj4(d3, W3l, W3h)), x7 = _mm256_sub_pd(s3, cmulconj4(d3, W3l, W3h));
        _mm256_storeu_pd((double*)p, x0); _mm256_storeu_pd((double*)(p + q), x1);
        _mm256_storeu_pd((double*)(p + 2 * q), x2); _mm256_storeu_pd((double*)(p + 3 * q), x3);
        _mm256_storeu_pd((double*)(p + 4 * q), x4); _mm256_storeu_pd((double*)(p + 5 * q), x5);
        _mm256_storeu_pd((double*)(p + 6 * q), x6); _mm256_storeu_pd((double*)(p + 7 * q), x7);
    }
    if (p != e) {
        cpx a0 = p[0], a1 = p[q], a2 = p[2 * q], a3 = p[3 * q], a4 = p[4 * q], a5 = p[5 * q], a6 = p[6 * q], a7 = p[7 * q];
        cpx v0 = _mm_add_pd(a0, a1), v1 = _mm_sub_pd(a0, a1);
        cpx v2 = _mm_add_pd(a2, a3), v3 = _mm_sub_pd(a2, a3);
        cpx r0 = _mm_add_pd(a4, a5), r1 = _mm_sub_pd(a4, a5);
        cpx r2 = _mm_add_pd(a6, a7), r3 = _mm_sub_pd(a6, a7);
        cpx s0 = _mm_add_pd(v0, v2), s2 = _mm_sub_pd(v0, v2);
        cpx s1 = _mm_add_pd(v1, cmulconj(v3, w2)), s3 = _mm_sub_pd(v1, cmulconj(v3, w2));
        cpx d0 = _mm_add_pd(r0, r2), d2 = _mm_sub_pd(r0, r2);
        cpx d1 = _mm_add_pd(r1, cmulconj(r3, w2)), d3 = _mm_sub_pd(r1, cmulconj(r3, w2));
        cpx x0 = _mm_add_pd(s0, d0), x4 = _mm_sub_pd(s0, d0);
        cpx x1 = _mm_add_pd(s1, cmulconj(d1, w1)), x5 = _mm_sub_pd(s1, cmulconj(d1, w1));
        cpx x2 = _mm_add_pd(s2, cmulconj(d2, w2)), x6 = _mm_sub_pd(s2, cmulconj(d2, w2));
        cpx x3 = _mm_add_pd(s3, cmulconj(d3, w3)), x7 = _mm_sub_pd(s3, cmulconj(d3, w3));
        p[0] = x0; p[q] = x1; p[2 * q] = x2; p[3 * q] = x3;
        p[4 * q] = x4; p[5 * q] = x5; p[6 * q] = x6; p[7 * q] = x7;
    }
}

static void difFlat(cpx* d, u32 n, u32 bb) {
    u32 bc = 1, bs = n >> 1, st = n;
    for (; bs >= 4; bc <<= 3, st = bs >> 2, bs >>= 3) {
        const u32 q = bs >> 2, base = bb * bc;
        u32 j = 0; cpx* s = d;
        if (base == 0) { bf8Fwd(s, q, twg(1), twg(2), twg(3)); j = 1; s += st; }
        else { for (; j != bc; ++j, s += st) {
            bf8Fwd(s, q, twg(base + j), twg(2 * (base + j)), twg(3 * (base + j)));
        }}
    }
    if (bs >= 2) {  // radix-8 残留: bs==2 -> 因子4 -> size-4 radix-4; bs==1 -> 平凡 size-2 不处理
...
static void ditFlat(cpx* d, u32 n, u32 bb) {
    u32 q = 1, bcOut = n >> 3;
    for (; (q << 3) <= n; q <<= 3, bcOut >>= 3) {
        const u32 st = q << 3, base = bb * bcOut, base2 = base << 1;
        u32 j = 0; cpx* s = d;
        if (base == 0) { bf8Inv(s, q, twg(1), twg(2), twg(3)); j = 1; s += st; }
        else { for (; j != bcOut; ++j, s += st) {
            bf8Inv(s, q, twg(base + j), twg(2 * (base + j)), twg(3 * (base + j)));
        }}
    }
    if ((q << 2) <= n) {  // radix-4 残留层
...
int main() {
    ...
    for (u32 ts : sizes) {
        alignas(16) double* buf = (double*)std::aligned_alloc(16, (size_t)ts*2*sizeof(double));
        alignas(16) double* ref = (double*)std::aligned_alloc(16, (size_t)ts*2*sizeof(double));
        for (u32 i=0;i<ts*2;i++){ double v=(double)(rng()&0xffff); buf[i]=v; ref[i]=v; }
        fft::resize(ts);
