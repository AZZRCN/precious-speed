...
static void mul_fft(const u64* a, int na, const u64* b, int nb, u64* c) {
    ...
    fprintf(stderr, "  [fft] u=%zu k=%d coeffs=%u lm=%u ts=%u  bytes=%.1fMiB\n",
            u, k, coeffs, lm, ts, (double)lm * 8 / 1048576.0);
}
...
int main() {
    ...
    fprintf(stderr,
        "V5P read=%.2f parse=%.2f | split=%.2f dif=%.2f pw=%.2f dit=%.2f merge=%.2f "
