#!/usr/bin/env python3
"""D7 正确性验证: 官方 26 例逐字节哈希 + 对黄金实现 div_orig 的压力测试。

与 d5_verify.py 同构, 仅指向 div_D7 (安全 in 选择)。
"""
import random
import subprocess
import sys
...
SRC = Path('/home/azzr/divbench/src/div_D7.cpp')
BIN = Path('/home/azzr/divbench/bin/div_D7')
...
def build():
    ...
    if r.returncode != 0:
        print('BUILD FAIL:\n' + r.stdout[-4000:])
        return False
    print('build div_D7 OK', flush=True)
    return True
    ...
    for g, n in GENS.items():
        ...
        for seed in range(1, n + 1):
            ...
            if r1 != 0 or r2 != 0 or a != b:
                ...
                Path(inf).rename(f'/home/azzr/divbench/logs/bad_d7_{g}_{seed}.in')
            else:
    ...
    for l2, qn in shapes:
        ...
        if len(pairs) >= 12:
            ...
            if r1 != 0 or r2 != 0 or a != b:
                ...
                Path(inf).rename(f'/home/azzr/divbench/logs/bad_t6d_{batch}.in')
            else:
                Path(inf).unlink(missing_ok=True)
            pairs = []
