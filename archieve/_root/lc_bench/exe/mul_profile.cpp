...
// ===== MUL_PROFILE phase instrumentation (analysis copy only) =====
#ifdef MUL_PROFILE
#include <chrono>
namespace mulprof {
    struct Rec { double ms = 0; long long n = 0; const char* name = nullptr; } recs[24];
    inline void reg(int i, const char* nm){ if(!recs[i].name) recs[i].name = nm; }
    struct Scope {
        int i; std::chrono::high_resolution_clock::time_point t0;
        Scope(int i):i(i){ t0 = std::chrono::high_resolution_clock::now(); }
        ~Scope(){ auto d = std::chrono::duration<double,std::milli>(std::chrono::high_resolution_clock::now()-t0).count(); recs[i].ms += d; recs[i].n++; }
    };
    void dump(){
        double tot = 0; for(int i=0;i<24;i++) if(recs[i].name) tot += recs[i].ms;
        fprintf(stderr, "MULPROF_TOTAL %.3f\n", tot); fflush(stderr);
        for(int i=0;i<24;i++) if(recs[i].name && recs[i].n)
            fprintf(stderr, "MULPROF %-12s %8.3f ms  (%lld calls, %7.3f avg)\n", recs[i].name, recs[i].ms, (long long)recs[i].n, recs[i].ms/recs[i].n);
        fflush(stderr);
    }
}
#define MPROF(i,nm) mulprof::reg(i,nm); mulprof::Scope _mp_##i(i);
#else
#define MPROF(i,nm)
#endif
// ===== end MUL_PROFILE =====
...
public:
  ...
  public:
    ...
    UnsignedInteger& operator*=(const UnsignedInteger& other) {
...
#ifdef MUL_PROFILE
        fprintf(stderr, "FLOAT_LEN %u  resultLen=%u\n", transformLength, resultLength);
#endif
        ...
        { MPROF(2,"dif_a") detail::T.decimationInFrequency(firstArray, transformLength); }
        { MPROF(3,"dif_b") detail::T.decimationInFrequency(secondArray, transformLength); }
        { MPROF(4,"dot") detail::T.frequencyDomainPointwiseMultiply(firstArray, secondArray, transformLength); }
        { MPROF(5,"dit") detail::T.decimationInTime(firstArray, transformLength); }
#else
...
int main() {
    ...
    ++iCursor;
    do {
        { MPROF(10,"parse") iCursor += (sa = *iCursor == '-'); readFromCursor(a); iCursor += (sb = *iCursor == '-'); readFromCursor(b); }
        { MPROF(11,"compute") a *= b; }
        // 负号条件：符号相异 && 结果非零 && 两操作数非零（与原作者 origin_multi 一致）
        { MPROF(12,"write") if (sa != sb && bool(a) && bool(b)) *oCursor++ = '-'; writeUnsigned(a); *oCursor++ = '\n'; }
    } while (--t);
