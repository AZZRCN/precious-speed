...
385663	2026/7/15 22:24:55	

Multiplication of Big Integers
	C++20	(Anonymous)	AC	36 ms	32.30 Mib
	Name	Status	Time	Memory
	example_00	AC	1 ms	0.80 Mib
	small_00	AC	20 ms	7.41 Mib
	medium_00	AC	18 ms	8.51 Mib
	medium_01	AC	17 ms	8.27 Mib
	medium_02	AC	27 ms	8.30 Mib
	large_00	AC	23 ms	10.12 Mib
	large_01	AC	24 ms	10.37 Mib
	large_02	AC	23 ms	9.22 Mib
	max_max_00	AC	35 ms	31.55 Mib
	max_max_01	AC	34 ms	31.55 Mib
	max_max_02	AC	34 ms	31.28 Mib
	max_max_03	AC	35 ms	31.72 Mib
	max_max_04	AC	34 ms	32.27 Mib
	max_max_05	AC	35 ms	30.79 Mib
	max_max_06	AC	34 ms	32.16 Mib
	max_max_07	AC	35 ms	30.37 Mib
	zero_00	AC	3 ms	0.79 Mib
	fft_killer_00	AC	34 ms	30.98 Mib
	fft_killer_01	AC	36 ms	32.30 Mib
	large_small_00	AC	33 ms	21.79 Mib
*/
/**
 * @file Integer.h
...
// OPT Phase2: 深度优先递归的叶子规模（复数个数取 2 的幂）
#ifndef MUL_FFT_LEAF_LOG
#define MUL_FFT_LEAF_LOG 10   // radix-4 每级吃 2 层，有效叶子 2^9
#endif
...
namespace profile_mul {
    inline double& slot(int i) { static double v[12] = {}; return v[i]; }
    inline const char* kNames[12] = {
        "parse", "fft_forward", "pointwise", "fft_inverse",
        "multiply_all", "write_convert", "fwrite", "buf_prep",
        "pack", "unpack_carry", "result_alloc", "(unused)"
    };
    struct Timer {
        int index;
    ...
    inline void dump() {
        const double nonfft = slot(4) - slot(1) - slot(2) - slot(3);
        const double accounted = slot(7) + slot(8) + slot(9) + slot(10);
        std::fprintf(stderr, "---- phase profile (ms) ----\n");
        for (int i = 0; i < 11; ++i)
            if (slot(i) > 0.0005) std::fprintf(stderr, "  %-14s %8.3f\n", kNames[i], slot(i));
        std::fprintf(stderr, "  %-14s %8.3f\n", "mul_nonfft", nonfft);
        std::fprintf(stderr, "  %-14s %8.3f\n", "  ..unaccounted", nonfft - accounted);
        std::fprintf(stderr, "  %-14s %8.3f\n", "SUM",
...
// OPT Phase3b: FFT 缓冲区分配器。
// 2^19 复数 = 8 MB/数组，两个数组 16 MB。按 4 KB 页首次触碰要 ~4000 次缺页，
// 每次约 0.5-1 us，合计 2-4 ms —— 在 28 ms 的总预算里不是小数目。
// 用 mmap 取 2 MB 对齐区域并 madvise(MADV_HUGEPAGE)，缺页降到个位数。
// MAP_ANONYMOUS 保证清零，故调用方首次分配后无需 memset。
// 缓冲区是进程生命期的缓存，扩容至多发生 log 次，故不回收（避免 munmap 开销）。
// 判题机的编译命令固定（-O2 -std=c++23 -march=native），无法外部传 -D，
// 故在 Linux 上默认开启大页路径；本地 Windows 构建自动走 #else 分支。
// 需要对照实验时用 -DMUL_NO_HUGEPAGE 关闭。
...
namespace detail {
#if defined(MUL_HUGEPAGE) && defined(__linux__)
    // 运行时探测 THP。判题机若配置成 never，madvise(MADV_HUGEPAGE) 静默无效，
    // 而裸 mmap 的缺页反而比 operator new 多 ~2100 次（实测 9517 vs 7369，慢 ~3.6%）。
    // 一次 open/read/close 约 10 us，换取“最坏情况不退化”的保证。
    inline bool hugePagesUsable() {
        static const bool ok = [] {
            const int fd = ::open("/sys/kernel/mm/transparent_hugepage/enabled", O_RDONLY);
            if (fd < 0)
                // ENOENT = 内核未编译 THP，只能回退；其余（容器里 /sys 常被限制读）
                // 无法判定，按“多半可用”处理：赌错只亏 3.6%，赌对省 26%。
                return errno != ENOENT;
            char buffer[128];
    ...
    struct TransformHelper {
        ...
        // 单组 radix-4 正向蝶形：作用于 g[0, 4q)，w = tw[2b]，w2 = tw[b]
        static inline void radix4Forward(__m128d* g, std::uint32_t q, __m128d w, __m128d w2) {
        ...
        // 奇数级收尾：blockSize == 1 的那一层仍走 radix-2
        void radix2TailForward(__m128d* dataArray, std::uint32_t transformSize, std::uint32_t blockBase) {
            const std::uint32_t pairCount = transformSize >> 1, twBase = blockBase * pairCount;
            std::uint32_t j = 0;
            __m128d* d = dataArray;
            if (twBase == 0) {
                const __m128d e = d[0], o = d[1];
                d[0] = _mm_add_pd(e, o), d[1] = _mm_sub_pd(e, o);
                j = 1, d += 2;
            }
            for (; j != pairCount; ++j, d += 2) {
                const __m128d ee = d[0], oo = complexMultiply(d[1], twiddleFactors[twBase + j]);
                d[0] = _mm_add_pd(ee, oo), d[1] = _mm_sub_pd(ee, oo);
            }
        }

        // OPT Phase2: 子数组编号 blockBase 在局部第 ℓ 层的全局块号为 blockBase * 2^ℓ + b，
        // 而 radix-4 每级恰好吃掉两层，故 groupCount == 2^ℓ，索引换成 blockBase*groupCount+b 即可。
        void dif4Flat(__m128d* dataArray, std::uint32_t transformSize, std::uint32_t blockBase) {
            const std::uint32_t stageCount = detail::log2(transformSize);
            std::uint32_t groupCount = 1;
            for (std::uint32_t q = transformSize >> 2; q; q >>= 2, groupCount <<= 2) {
                const std::uint32_t span = q << 2, twBase = blockBase * groupCount;
                for (std::uint32_t b = 0; b != groupCount; ++b) {
                    const std::uint32_t bg = twBase + b;
                    radix4Forward(dataArray + std::uintptr_t(b) * span, q,
                                  twiddleFactors[bg << 1], twiddleFactors[bg]);
                }
                if (q == 1) break;
            }
            if (stageCount & 1) radix2TailForward(dataArray, transformSize, blockBase);
        }

        void dif4Recursive(__m128d* dataArray, std::uint32_t transformSize, std::uint32_t blockBase) {
            if (transformSize <= (1u << MUL_FFT_LEAF_LOG)) {
                dif4Flat(dataArray, transformSize, blockBase);
                return;
            }
            const std::uint32_t q = transformSize >> 2;
            radix4Forward(dataArray, q, twiddleFactors[blockBase << 1], twiddleFactors[blockBase]);
            const std::uint32_t childBase = blockBase << 2;
            dif4Recursive(dataArray, q, childBase);
            dif4Recursive(dataArray + q, q, childBase | 1);
            dif4Recursive(dataArray + 2 * q, q, childBase | 2);
            dif4Recursive(dataArray + 3 * q, q, childBase | 3);
        }

        void decimationInFrequency(__m128d* dataArray, std::uint32_t transformSize) {
            PROF(1);
            dif4Recursive(dataArray, transformSize, 0);
        }
        ...
        // 单组 radix-4 逆向蝶形：作用于 g[0, 4q)，w = tw[2b]，w2 = tw[b]
        static inline void radix4Inverse(__m128d* g, std::uint32_t q, __m128d w, __m128d w2) {
            const __m128d w3 = complexMultiply(w2, w);
            const __m256d w1v = _mm256_set_m128d(w, w), w2v = _mm256_set_m128d(w2, w2), w3v = _mm256_set_m128d(w3, w3);
        ...
        void radix2TailInverse(__m128d* dataArray, std::uint32_t transformSize, std::uint32_t blockBase) {
            const std::uint32_t pairCount = transformSize >> 1, twBase = blockBase * pairCount;
            std::uint32_t j = 0;
            __m128d* d = dataArray;
            if (twBase == 0) {
            ...
            for (; j != pairCount; ++j, d += 2) {
                const __m128d ee = d[0], oo = d[1];
                d[0] = _mm_add_pd(ee, oo);
                d[1] = complexMultiplyConjugate(_mm_sub_pd(ee, oo), twiddleFactors[twBase + j]);
            }
        }

        void dit4Flat(__m128d* dataArray, std::uint32_t transformSize, std::uint32_t blockBase) {
            const std::uint32_t stageCount = detail::log2(transformSize);
            if (stageCount & 1) radix2TailInverse(dataArray, transformSize, blockBase);
            std::uint32_t groupCount = transformSize >> ((stageCount & 1) ? 3 : 2);
            for (std::uint32_t q = (stageCount & 1) ? 2u : 1u; q <= (transformSize >> 2);
                 q <<= 2, groupCount >>= 2) {
                const std::uint32_t span = q << 2, twBase = blockBase * groupCount;
                for (std::uint32_t b = 0; b != groupCount; ++b) {
                    const std::uint32_t bg = twBase + b;
                    radix4Inverse(dataArray + std::uintptr_t(b) * span, q,
                                  twiddleFactors[bg << 1], twiddleFactors[bg]);
                }
            }
        }

        void dit4Recursive(__m128d* dataArray, std::uint32_t transformSize, std::uint32_t blockBase) {
            if (transformSize <= (1u << MUL_FFT_LEAF_LOG)) {
                dit4Flat(dataArray, transformSize, blockBase);
                return;
            }
            const std::uint32_t q = transformSize >> 2;
            const std::uint32_t childBase = blockBase << 2;
            dit4Recursive(dataArray, q, childBase);
            dit4Recursive(dataArray + q, q, childBase | 1);
            dit4Recursive(dataArray + 2 * q, q, childBase | 2);
            dit4Recursive(dataArray + 3 * q, q, childBase | 3);
            radix4Inverse(dataArray, q, twiddleFactors[blockBase << 1], twiddleFactors[blockBase]);
        }

        void decimationInTime(__m128d* dataArray, std::uint32_t transformSize) {
            PROF(3);
            dit4Recursive(dataArray, transformSize, 0);
        }
    ...
    public:
        static std::uint32_t* allocate(std::uint32_t size) {
            ...
            if (buckets[idx]) {
                ...
                // OPT Phase3b: ≥2 MB 的大块（结果数组、2M 位操作数）走大页。
                // 这些数组只被线性扫描一次，4 KB 页下缺页与 TLB miss 的开销
                // 比实际计算还大；deallocate 只回收到自由链表、不还给 OS，
                // 故 mmap 内存与 operator new 内存混用是安全的。
...
public:
  ...
  public:
    ...
    UnsignedInteger& operator*=(const UnsignedInteger& other) {
        ...
        {
            PROF(7);
            // 实测结论：手写 calloc + 跳过尾部 memset 反而慢 3.6%（VM 冷跑 28.23 vs 27.25），
            // 因为 GCC 已把 new T[n]() 识别为 calloc 语义并消掉了后续冗余 memset，
            // 手写版反倒多摸了 2151 个页（缺页 9450 vs 7299）。故默认保留原写法。
            // 唯一仍值得试的是大页：16 MB 按 4 KB 页要 ~4000 次缺页。
#if defined(MUL_HUGEPAGE) && defined(__linux__)
            // 分支由进程内的静态探测一次性决定，故同一份缓冲区始终由同一条路径
            // 分配/释放，不存在 delete[] 到 mmap 指针的混用。
        ...
        detail::T.resize(transformLength);
        detail::T.decimationInFrequency(firstArray, transformLength), detail::T.decimationInFrequency(secondArray, transformLength);
        { PROF(2); detail::T.frequencyDomainPointwiseMultiply(firstArray, secondArray, transformLength); }
        detail::T.decimationInTime(firstArray, transformLength);
#else
...
#endif
#if defined(__AVX2__)
        UnsignedInteger result = [&] { PROF(10); return UnsignedInteger(resultLength, resultLength); }();
        std::uint64_t carry = 0;
        {
        PROF(9);
...
namespace {
#if defined(MUL_HUGEPAGE) && defined(__linux__)
    // 输出缓冲区 2 MB 对齐，便于 madvise(MADV_HUGEPAGE) 生效（见 main 开头）
    ...
    static const char* iCursor = []() -> const char* {
        ...
        // 输入是 file-backed 映射，THP 不适用；改用一次系统调用批量预填页表，
        // 取代读取时逐页的 ~1000 次缺页陷入。
...
int main() {
    ...
    { PROF(6); std::fwrite(oBuffer, sizeof(char), oCursor - oBuffer, stdout); }
#ifdef PROFILE_MUL
    profile_mul::dump();
#endif
    return 0;
