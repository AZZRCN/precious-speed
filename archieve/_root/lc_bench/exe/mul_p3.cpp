...
// ===== MUL_PROFILE (analysis copy only) =====
#ifdef MUL_PROFILE
#include <chrono>
namespace mulprof {
  double t[16]={0}; long long n[16]={0}; const char* nm[16]={0};
  inline void reg(int i,const char* s){ if(!nm[i]) nm[i]=s; }
  inline void add(int i,double ms){ t[i]+=ms; n[i]++; }
  inline double now(){ return std::chrono::duration<double,std::milli>(std::chrono::high_resolution_clock::now().time_since_epoch()).count(); }
  void dump(){ for(int i=0;i<16;i++) if(nm[i]&&n[i]) fprintf(stderr,"MULPROF %-10s %8.3f ms (%lld)\n",nm[i],t[i],(long long)n[i]); fflush(stderr); }
}
#define MPROF_BEG(i,s) mulprof::reg(i,s); double _t##i=mulprof::now();
#define MPROF_END(i) mulprof::add(i,mulprof::now()-_t##i);
#else
#define MPROF_BEG(i,s)
#define MPROF_END(i)
#endif
// ===== end MUL_PROFILE =====
