// decN_sb.cpp —— 参数化十进制大基数 schoolbook 除法原型
//   -DDBASE=19  : 基数 10^19 (理论最优, 但 19 不是 4 的倍数, 无法与 D47 的 base-1e4 直接互转)
//   -DDBASE=16  : 基数 10^16 = (10^4)^4, **与 D47 base-1e4 精确整除对齐**, 打包/解包纯线性
// 实测地基 (VM, rnz_01, 同一批 case):
//   D47 base-1e4 + AVX2 : 9.07 instr / 5.34 cyc 每 limb-op
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cstdint>
...
#endif
#if DBASE == 19
static const u64 DV   = 10000000000000000000ull;
static const u64 DN   = 10000000000000000000ull;   // DV << SHC
static const u64 VN   = 0xd83c94fb6d2ac34aull;     // invert_limb(DN)
#define SHC 0
#elif DBASE == 16
static const u64 DV   = 10000000000000000ull;
static const u64 DN   = 10240000000000000000ull;   // 10^16 << 10
static const u64 VN   = 0xcd2b297d889bc2b6ull;
#define SHC 10
#else
#error "DBASE must be 16 or 19"
#endif
static const u64 D19  = DV;                        // 沿用旧名, 少改代码
static const u64 NEGD = 0ull - DV;
...
// 128 位除以常量 10^DBASE。SHC!=0 时先把被除数左移到归一化位置
static inline void divrem_D19(u64 &q, u64 &r, u64 uh, u64 ul)
{
#if SHC == 0
    udiv_qrnnd_preinv(q, r, uh, ul, DN, VN);
#else
    u64 h2 = (uh << SHC) | (ul >> (64 - SHC));
    u64 l2 = ul << SHC;
    u64 rr;
    udiv_qrnnd_preinv(q, rr, h2, l2, DN, VN);
    r = rr >> SHC;
#endif
}
...
{
    size_t first = n % DBASE; if (!first) first = DBASE;
    size_t nl = 1 + (n - first) / DBASE;
    size_t idx = nl;
    out[--idx] = parse_chunk(s, first);
    size_t pos = first;
    while (pos < n) { out[--idx] = parse_chunk(s + pos, DBASE); pos += DBASE; }
...
static inline void writeD(char *p, u64 v)
{
    for (int i = 0; i < DBASE / 2; i++)
    {
        u32 dd = u32(v % 100); v /= 100;
        p[DBASE - 2 - 2 * i] = DIG2[2 * dd]; p[DBASE - 1 - 2 * i] = DIG2[2 * dd + 1];
    }
#if (DBASE & 1)
    p[0] = char('0' + v);
#endif
...
static size_t dec19str(const u64 *v, size_t n, char *out)
{
    ...
    if (t == 0) *p++ = '0';
           ...
           while (bl) *p++ = b[--bl]; }
    for (size_t i = n - 1; i-- > 0;) { writeD(p, v[i]); p += DBASE; }
    return (size_t)(p - out);
}
...
int main(int argc, char **argv)
{
    ...
    for (size_t i = 0; i < T; i++)
    {
        ...
        dec4 += qn * n2;
        size_t m1 = (la + DBASE - 1) / DBASE, m2 = (lb + DBASE - 1) / DBASE;
    ...
    { size_t N = std::max<size_t>(ta.size(), 1);
      printf("cases=%zu (skipped %zu)\n  base-1e4  limb-ops=%zu (x8.1 = %.3g Ir)\n"
             ...
             "  => 固定/内环 = %.2f\n",
             ta.size(), skipped, dec4, dec4 * 8.1, (int)DBASE, dec19, (double)dec4 / (double)std::max<size_t>(dec19, 1),
