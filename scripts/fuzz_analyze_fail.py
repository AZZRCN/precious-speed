...
import sys
target = sys.argv[1] if len(sys.argv) > 1 else "moptm_baseline"
...
for i, (a_d, b_d) in enumerate(tests):
    ...
    try:
        ...
        q_ok = False
        r_ok = False
    if q_ok: q_ok_count += 1
    if r_ok: r_ok_count += 1
    if not q_ok or not r_ok:
        fail += 1
        print(f"FAIL #{i}: a={a_d} b={b_d} q_ok={q_ok} r_ok={r_ok}")
        if not q_ok:
            print(f"  correct_q_len={len(sq)} got_q_len={len(mq)}")
            print(f"  correct_q_head={sq[:40]}")
            print(f"  got_q_head    ={mq[:40]}")
        sys.stdout.flush()

print(f"\\n=== Summary ({target}) ===")
print(f"  q_ok: {q_ok_count}/{len(tests)}")
print(f"  r_ok: {r_ok_count}/{len(tests)}")
print(f"  both_ok: {len(tests)-fail}/{len(tests)}")
...
for name, flags in [
    ("cyclic_only", ""),
    ("gmp_only", "-DUSE_GMP_NEWTON -DDISABLE_2NXN_CYCLIC"),
]:
    exe = f"moptm_{name}"
