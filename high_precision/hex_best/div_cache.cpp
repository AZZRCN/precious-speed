...
int main() {
    ...
    int len = 0;
    // DEC ADD 风格输入优化: mmap 文件 + MADV 顺序/预取/巨页。
    // 但 HEX 的 hexfull/hexpart 反向读 16B, 零拷贝直用 mmap 会因首 token 反向越界 SEGV,
    // 故拷入带 PAD 的 inbuf_ (INCAP 余量需够)。仅改 I/O; 计算/算法零改动。
    // cycles 为 Intel VM 代理值, AMD 真值以 LC 提交回执为准。
    struct stat st;
    if (fstat(0, &st) == 0 && st.st_size > 0 && (size_t)st.st_size <= (size_t)INCAP - PAD) {
        ...
        if (m != MAP_FAILED) {
            ...
            madvise(m, st.st_size, MADV_HUGEPAGE);
            memcpy(inbuf_ + PAD, m, st.st_size);
            munmap(m, st.st_size);
            inbuf = inbuf_ + PAD;
            len = (int)st.st_size;
        }
    }
    if (len == 0) {   // 回退: 管道/超大 (按 len 推进, 避免大文件多次 read 覆盖)
        for (;;) {
            long r = read(0, inbuf_ + PAD + len, INCAP - len);
            if (r <= 0) break;
            len += (int)r;
        }
        inbuf = inbuf_ + PAD;
