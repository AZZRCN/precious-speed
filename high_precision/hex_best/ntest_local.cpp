...
int main(){
    ...
    const char* p=buf;
    while(*p==' '||*p=='\n')++p;
    const char* a0=p; while(*p!=' '&&*p!='\n'&&*p)++p; int la=(int)(p-a0);
    while(*p==' ')++p; const char* b0=p; while(*p!='\n'&&*p)++p; int lb=(int)(p-b0);
    int na=hex2limbs(a0,la,A); int nb=hex2limbs(b0,lb,B);
    while(na>1&&A[na-1]==0)--na; while(nb>1&&B[nb-1]==0)--nb;
    int n=nb;
    u64* Qo=WORK+0; u64* Ro=WORK+(na-nb+4);
    wp=WORK+2*na+64;
    newton_divide(A,na,B,nb,Qo,Ro);
    char o[4000000]; limbs2hex(Qo,na-nb+1,o); printf("Q=%s\n",o);
    limbs2hex(Ro,n,o); printf("R=%s\n",o);
    return 0;
}
