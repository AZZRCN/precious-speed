...
290337	2025/6/2 10:27:39	

Multiplication of Hex Big Integers
	C++20	(Anonymous)	AC	35 ms	27.05 Mib
	Name	Status	Time	Memory
	example_00	AC	1 ms	0.62 Mib
	fft_killer_00	AC	34 ms	26.89 Mib
	fft_killer_01	AC	35 ms	26.63 Mib
	large_00	AC	18 ms	5.34 Mib
	large_01	AC	21 ms	5.05 Mib
	large_02	AC	21 ms	5.20 Mib
	large_small_00	AC	31 ms	18.11 Mib
	max_max_00	AC	34 ms	26.54 Mib
	max_max_01	AC	35 ms	26.89 Mib
	max_max_02	AC	34 ms	26.23 Mib
	max_max_03	AC	33 ms	27.05 Mib
	max_max_04	AC	34 ms	26.89 Mib
	max_max_05	AC	34 ms	26.64 Mib
	max_max_06	AC	34 ms	26.39 Mib
	max_max_07	AC	35 ms	27.02 Mib
	medium_00	AC	8 ms	3.43 Mib
	medium_01	AC	9 ms	3.73 Mib
	medium_02	AC	8 ms	3.58 Mib
	power_00	AC	8 ms	3.48 Mib
	power_01	AC	10 ms	3.74 Mib
	small_00	AC	30 ms	3.73 Mib
	zero_00	AC	3 ms	0.73 Mib
*/

//only for test
//from the submit https://loj.ac/s/1997515
//from the submit https://judge.yosupo.jp/submission/208479
#include <bits/stdc++.h>
...
struct cpx{
	...
	cpx()=default;
	cpx(__m128d vv):v(vv){}
	cpx(f64 x,f64 y=0.0):v(_mm_set_pd(y,x)){}
	...
	friend cpx subI(cpx a,cpx b){return {b.imag()-a.imag(),a.real()-b.real()};}
	cpx operator-(){return -v;}
    f64 real(){return v[0];}
    f64 imag(){return v[1];}
};
...
inline cpx mulY(cpx x,cpx y){
	return _mm_fmadd_pd(_mm_unpacklo_pd(x.v,x.v),y.v,_mm_unpackhi_pd(x.v,x.v)*_mm_permute_pd(y.v,1));
}
inline cpx Wn(ldb a){return {f64(std::cos(a)),f64(std::sin(a))};}
inline cpx conj(cpx x){return {x.real(),-x.imag()};}
struct ffter{
    std::vector<cpx> wn;
    ffter():wn{1.}{}
    void reserve(idt l){
...
void split_b2(u64*_f,f64*g,idt n,int k){
    ...
    while(w>0){g[j++]=i64(tmp&msk),tmp>>=k,w-=k;}
}
void merge_b2(u64*f,f64*g,idt n,int k){
    ...
    __uint128_t tmp=0;
    while(i<n){
        while(w<64){tmp+=__uint128_t(u64(i64(g[j++]+0.5)))<<w,w+=k;}
        if(w>=64){f[i++]=u64(tmp),tmp>>=64,w-=64;}
    }
}
...
struct _buint{
    ...
    _buint _rmul_fft(const _buint&b)const{
        ...
        static constexpr auto gkk=[]{
            std::array<idt,11> ar={};
            for(idt i=0;i<10;++i){ar[i]=idt(19-i)<<(2*i+4);}
            ar[10]=idt(-1);
            return ar;
        }();
        int kk=0;
...
void solve(){
    ...
    while(T--){
        ...
        int sign=0;
        if(av.front()=='-'){sign^=1,av.remove_prefix(1);}
        if(bv.front()=='-'){sign^=1,bv.remove_prefix(1);}
        fr_str(A,av),fr_str(B,bv);
        auto r=A*B;
        if(sign&&r){cout<<'-';}
        to_str(r,a);
        cout<<a<<'\n';
