...
// ---- PROF: 分段计时 (仅本地参照版, 不提交) ----
using pclk = std::chrono::steady_clock;
static double P_read, P_parse, P_split, P_dif, P_pw, P_dit, P_merge, P_emit, P_write;
static pclk::time_point PT_;
#define PB() PT_ = pclk::now()
#define PE(acc) acc += std::chrono::duration<double, std::milli>(pclk::now() - PT_).count()
...
namespace fft {
using cpx = __m128d;
// twiddle 表转静态 BSS: 原来 new cpx[n>>1] 是 4 MiB 堆分配, 走小页 -> ~1000 次 fault。
alignas(HP) static __m128d twbuf[LMMAX >> 1];
static cpx* tw = twbuf;
static u32 twlen = 0;
...
static void resize(u32 n) {  // n = 复数点数
    ...
    alignas(64) static cpx base[1u << 12];   // halfSize <= 2^10 (n <= 2^21)
...
static void split_b2(const u64* src, double* g, size_t n, int k, size_t lm) {
    ...
    if (lm > total) std::memset(g + total, 0, (lm - total) * 8);
}
...
static void mul_fft(const u64* a, int na, const u64* b, int nb, u64* c) {
    ...
    const u32 ts = lm >> 1;
    fft::resize(ts);
    PB(); fft::difRec((fft::cpx*)FB, ts, 0); PE(P_dif);
    if (same) {
        PB(); fft::pointwiseSq((fft::cpx*)FB, ts); PE(P_pw);
    } else {
        PB(); fft::difRec((fft::cpx*)GB, ts, 0); PE(P_dif);
        PB(); fft::pointwise((fft::cpx*)FB, (fft::cpx*)GB, ts); PE(P_pw);
    }
    PB(); fft::ditRec((fft::cpx*)FB, ts, 0); PE(P_dit);
    PB(); merge_b2(c, FB, u, k); PE(P_merge);
    fprintf(stderr, "  [fft] u=%zu k=%d lm=%u ts=%u bytes=%.1fMiB\n", u, k, lm, ts, (double)lm*8/1048576.0);
}
...
int main() {
    ...
    hugify(B, sizeof B);
    hugify(Rr, sizeof Rr);
    hugify(fft::twbuf, sizeof fft::twbuf);
    int len = 0;
    PB();
    for (;;) {
    ...
    PE(P_write);
    {
        double tot = std::chrono::duration<double, std::milli>(pclk::now() - TT0).count();
        fprintf(stderr,
            "V8P read=%.2f parse=%.2f | split=%.2f dif=%.2f pw=%.2f dit=%.2f merge=%.2f "
            "| emit=%.2f write=%.2f || FFTCORE=%.2f MULALL=%.2f TOTAL=%.2f\n",
            P_read, P_parse, P_split, P_dif, P_pw, P_dit, P_merge, P_emit, P_write,
            P_dif + P_pw + P_dit, P_split + P_dif + P_pw + P_dit + P_merge, tot);
    }
    return 0;
}
