...
int main() {
    ...
    for (uint32_t t = 0; t < T; ++t) {
        ...
        if (__builtin_expect(eb < 64, 1)) {
            ...
            if (__builtin_expect((la | lb) <= 15, 1)) {
                // 超快路径: 双方 <=15 位 -> 有符号 64 位算术
                uint64_t ua = hexpart(aend, la), ub = hexpart(bend, lb);
                int64_t ma64 = -(int64_t)nga, mb64 = -(int64_t)ngb;
                int64_t x = ((int64_t)ua ^ ma64) - ma64;
                int64_t y = ((int64_t)ub ^ mb64) - mb64;
                int64_t s = x + y;
                int64_t msk = s >> 63;
                uint64_t mg = (uint64_t)((s ^ msk) - msk);
                int neg = (int)(msk & 1);
                uint64_t bits64 = 64 - _lzcnt_u64(mg);
                int d64 = (int)((bits64 + 3) >> 2); d64 += (d64 == 0);
                *out = '-'; out += neg;
                hex16_store(mg << (64 - 4 * d64), out);
                out += d64;
                *out++ = '\n';
                p += eb;
                while (*p <= ' ') ++p;
