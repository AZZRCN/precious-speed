...
constexpr size_t HP = 1u << 21;   // 2 MiB 透明大页
// THP=madvise 时内核不主动给 2 MiB 页, 必须显式申请; LC THP=always 无害, never 退化无害。
inline void hugify(void* p, size_t n) {
#ifdef MADV_HUGEPAGE
...
constexpr int INCAP = 9 << 20;
constexpr int OUTCAP = 10 << 20;
constexpr int MAXC = 100010;          // 1.6M hex / 16
#ifndef BZ_CUTOFF
...
alignas(HP) char inbuf_[PAD + INCAP + PAD];
alignas(HP) char outbuf[OUTCAP + 256];
char* const inbuf = inbuf_ + PAD;
alignas(HP) u64 A[MAXC + 8], B[MAXC + 8];
alignas(HP) u64 Qout[MAXC + 8], Rout[MAXC + 8];
alignas(HP) u64 AN[2 * MAXC + 16], BN[MAXC + 8];  // Knuth D 归一化缓冲
alignas(HP) u64 AS[MAXC + 8192], BS[MAXC + 8192]; // BZ 顶层 shift 缓冲 (块长 n 可略大于 nb)
alignas(HP) u64 VB[MAXC + 8192];                  // Barrett 倒数 V 的低 n limbs (V = 2^64n + VB)
alignas(HP) u64 WORK[3200000];                    // BZ/Barrett 工作栈 (~25.6MB), 峰值约 16*n
u64* wp = WORK;

constexpr u32 LMMAX = 1u << 20;
alignas(HP) double FB[LMMAX], GB[LMMAX];
...
alignas(64) constexpr MaskTab MT{};

inline __m128i a2n(__m128i v) {
...
inline u64 hexfull(const char* end) {
...
inline u64 hexpart(const char* end, int L) {
    __m128i nib = a2n(_mm_loadu_si128((const __m128i*)(end - 16)));
...
inline void hex16_store(u64 val, char* out) {
...
inline int tok_len(const char* p) {
...
inline int parse_limbs(const char* s, int n, u64* out) {
    int nc = (n + 15) >> 4;
    const char* end = s + n;
    int full = nc - 1;
...
inline char* put_u64(char* out, u64 v) {
    int d = v ? 16 - (int)(_lzcnt_u64(v) >> 2) : 1;
    hex16_store(v << (64 - 4 * d), out);
    return out + d;
}
inline char* put_big(char* out, const u64* V, int n) {
    while (n > 1 && V[n - 1] == 0) --n;
...
alignas(64) __m128d twbase[1u << 12];
u32 twHalfLog = 0, twHalfSize = 0, twHalfMask = 0, twN = 0;
...
inline cpx twg(u32 i) {
    return cmul(twbase[i & twHalfMask], twbase[twHalfSize | (i >> twHalfLog)]);
}
// 高位分量提到循环外时用: 低位 lo, 高位分量 hi 已备好
inline cpx twlo(u32 i, cpx hi) { return cmul(twbase[i & twHalfMask], hi); }
inline cpx twhi(u32 i) { return twbase[twHalfSize | (i >> twHalfLog)]; }
// tw[2i+1] == tw[2i] * i (dump 验证的恒等式) -> 省一次 load + 一次复乘
inline cpx mulI(cpx z) {   // [re,im] -> [-im,re]
    return _mm_xor_pd(_mm_shuffle_pd(z, z, 1), _mm_set_pd(0.0, -0.0));
}

inline void bfPlain(cpx* s, u32 bs) {
    cpx* e = s + bs;
    cpx* p = s;
    for (; p + 2 <= e; p += 2) {
...
inline void bfFwd(cpx* s, u32 bs, cpx w) {
...
inline void bfInv(cpx* s, u32 bs, cpx w) {
    const __m256d w256 = _mm256_set_m128d(w, w);
...
inline void bf2Fwd(cpx* s, u32 q, cpx w0, cpx w1, cpx w2) {
    const u32 bs = q << 1;
...
inline void bf2FwdOne(cpx* s, u32 q, cpx w2) {  // w0 == w1 == 1
    const u32 bs = q << 1;
    const __m256d W2 = BC4(w2), W2s = _mm256_permute_pd(W2, 0x5);
    cpx* e = s + q;
    cpx* p = s;
...
inline void bf2Inv(cpx* s, u32 q, cpx w0, cpx w1, cpx w2) {
    const u32 bs = q << 1;
...
inline void bf2InvOne(cpx* s, u32 q, cpx w2) {  // w0 == w1 == 1
    const u32 bs = q << 1;
    const __m256d W2 = BC4(w2), W2l = _mm256_unpacklo_pd(W2, W2), W2h = _mm256_unpackhi_pd(W2, W2);
    cpx* e = s + q;
    cpx* p = s;
...
void difFlat(cpx* d, u32 n, u32 bb) {
...
void ditFlat(cpx* d, u32 n, u32 bb) {
    u32 q = 1, bcOut = n >> 2;
...
void difRec(cpx* d, u32 n, u32 bb) {
...
void ditRec(cpx* d, u32 n, u32 bb) {
...
void pointwise(cpx* F, cpx* G, u32 n) {
...
void split_b2(const u64* src, double* g, size_t n, int k) {
...
void merge_b2(u64* f, const double* g, size_t n, int k) {
    size_t i = 0, j = 0;
    int w = 0;
    u128 tmp = 0;
    while (i < n) {
...
inline int pick_k(size_t u) {
    const size_t gk[11] = {19ull << 4,  18ull << 6,  17ull << 8,  16ull << 10,
...
void mul_bf(const u64* a, int na, const u64* b, int nb, u64* c) {
...
void mul_fft(const u64* a, int na, const u64* b, int nb, u64* c) {
...
// c[0..na+nb) = a*b
void mulg(const u64* a, int na, const u64* b, int nb, u64* c) {
...
constexpr u32 FMCAP = 1u << 20;
struct FixedFFT { size_t u; u32 lm, ts; int k; bool ok; };
alignas(HP) double FMG1[FMCAP], FMG2[FMCAP];
FixedFFT FF1, FF2;

void fm_prep(FixedFFT& F, double* G, const u64* b, int nb, int na) {
    F.ok = false;
...
void fm_mul(const FixedFFT& F, double* G, const u64* a, int na, u64* c) {
...
// ============================ 基本 limb 运算 ============================
inline int cmpn(const u64* a, const u64* b, int n) {
    for (int i = n - 1; i >= 0; --i)
        if (a[i] != b[i]) return a[i] > b[i] ? 1 : -1;
    return 0;
}
inline u64 addn(u64* a, const u64* b, int n) {
    unsigned char c = 0;
    for (int i = 0; i < n; ++i) c = _addcarry_u64(c, a[i], b[i], (unsigned long long*)&a[i]);
    return c;
}
inline u64 subn(u64* a, const u64* b, int n) {
    unsigned char c = 0;
...
// ============================ Knuth D ============================
void knuthD(const u64* U, int mn, const u64* V, int n, u64* Qo, u64* Ro) {
    const int m = mn - n;
...
// ============================ Burnikel-Ziegler ============================
void div_3n_2n(const u64* Ain, const u64* Bin, int n, u64* Q, u64* R);

// A: 2n limbs, B: n limbs (归一化: B[n-1] 最高位为 1), 且 A < B*2^(64n)
// Q: n limbs, R: n limbs
void div_2n_1n(const u64* Ain, const u64* Bin, int n, u64* Q, u64* R) {
    if (n < BZ_CUTOFF || (n & 1)) {
        u64* Qt = wp; wp += n + 1;
...
#define INV_BASE 48
#endif
void invertappr(const u64* d, int n, u64* v) {
    if (n <= INV_BASE) {
        u64* U = wp; wp += 2 * n + 2;
        u64* Qt = wp; wp += n + 2;
        u64* Rt = wp; wp += n + 2;
...
void bz_divide(const u64* Ai, int na, const u64* Bi, int nb, u64* Q, u64* R) {
...
inline int mag_cmp(const u64* a, int na, const u64* b, int nb) {
    if (na != nb) return na > nb ? 1 : -1;
    return cmpn(a, b, na);
}

// ============================ divrem_1 ============================
u64 divrem_1(const u64* a, int na, u64 d, u64* q) {
