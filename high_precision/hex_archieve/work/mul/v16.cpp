...
static void pointwiseSq(cpx* F, u32 n) {
    ...
    struct MR_Tw {                       // 顶层蝶形旋转因子缓存 (factor-3 / factor-5 各自独立缓存)
        // factor-3: W_{3m}^c / W_{3m}^{2c}
        std::vector<cpx> t3a, t3b;
        size_t cached3 = 0;
        // factor-5: W_{5m}^c / W_{5m}^{2c} / W_{5m}^{3c} / W_{5m}^{4c}
        std::vector<cpx> t5a, t5b, t5c, t5d;
        size_t cached5 = 0;
        void ensure3(size_t m) {
            if (cached3 == m && !t3a.empty()) return;
            t3a.resize(m); t3b.resize(m);
            const double base = -3.14159265358979323846 * 2.0 / (3.0 * (double)m); // -2pi/(3m)
            for (size_t c = 0; c < m; ++c) {
                double a1 = base * (double)c, a2 = base * (2.0 * (double)c);
                t3a[c] = mkc(std::cos(a1), std::sin(a1));   // W_{3m}^c
                t3b[c] = mkc(std::cos(a2), std::sin(a2));   // W_{3m}^{2c}
            }
            cached3 = m;
        }
        void ensure5(size_t m) {
            if (cached5 == m && !t5a.empty()) return;
            t5a.resize(m); t5b.resize(m); t5c.resize(m); t5d.resize(m);
            const double base = -3.14159265358979323846 * 2.0 / (5.0 * (double)m); // -2pi/(5m)
            for (size_t c = 0; c < m; ++c) {
                double a = base * (double)c;
                t5a[c] = mkc(std::cos(a),      std::sin(a));      // W_{5m}^c
                t5b[c] = mkc(std::cos(2*a),    std::sin(2*a));    // W_{5m}^{2c}
                t5c[c] = mkc(std::cos(3*a),    std::sin(3*a));    // W_{5m}^{3c}
                t5d[c] = mkc(std::cos(4*a),    std::sin(4*a));    // W_{5m}^{4c}
            }
            cached5 = m;
        }
    };
    ...
    static void dif3StageR(cpx* b0, u32 m) {
        ...
        for (; c + 2 <= m; c += 2) {                       // 一次 2 个复数 (AVX2)
            ...
            __m256d w1 = _mm256_loadu_pd((const double*)(p1 + c));
            __m256d w2 = _mm256_loadu_pd((const double*)(p2 + c));
        ...
        for (; c < m; ++c) {
            cpx a0 = b0[c], a1 = B1[c], a2 = B2[c];
            cpx s = _mm_add_pd(a1, a2), d = _mm_sub_pd(a1, a2);
            cpx h = _mm_sub_pd(a0, _mm_mul_pd(s, _mm_set1_pd(0.5)));
            cpx g = cscale(mulI(d), SQRT3_DIV2);
            b0[c] = _mm_add_pd(a0, s);
            B1[c] = cmul(_mm_sub_pd(h, g), p1[c]);
            B2[c] = cmul(_mm_add_pd(h, g), p2[c]);
        }
    }
    static void idit3StageR(cpx* b0, u32 m) {
        ...
        for (; c < m; ++c) {
            cpx t0 = b0[c];
            cpx x1 = cmulconj(B1[c], p1[c]), x2 = cmulconj(B2[c], p2[c]);
            cpx s = _mm_add_pd(x1, x2), d = _mm_sub_pd(x1, x2);
            cpx h = _mm_sub_pd(t0, _mm_mul_pd(s, _mm_set1_pd(0.5)));
            cpx g = cscale(mulI(d), SQRT3_DIV2);
            b0[c] = _mm_add_pd(t0, s);
            B1[c] = _mm_add_pd(h, g);
            B2[c] = _mm_sub_pd(h, g);
        }
    }
    ...
    static void dif5StageR(cpx* b0, u32 m) {
        g_mr_tw.ensure5(m);
        const cpx* p1 = g_mr_tw.t5a.data(); const cpx* p2 = g_mr_tw.t5b.data();
        const cpx* p3 = g_mr_tw.t5c.data(); const cpx* p4 = g_mr_tw.t5d.data();
        ...
        for (; c + 2 <= m; c += 2) {
            ...
            _mm256_storeu_pd((double*)(B1+c), cmulv(_mm256_sub_pd(u1,v1), _mm256_loadu_pd((const double*)(p1+c))));
            _mm256_storeu_pd((double*)(B2+c), cmulv(_mm256_sub_pd(u2,v2), _mm256_loadu_pd((const double*)(p2+c))));
            _mm256_storeu_pd((double*)(B3+c), cmulv(_mm256_add_pd(u2,v2), _mm256_loadu_pd((const double*)(p3+c))));
            _mm256_storeu_pd((double*)(B4+c), cmulv(_mm256_add_pd(u1,v1), _mm256_loadu_pd((const double*)(p4+c))));
        }
        const double c1 = R5_C1, s1 = R5_S1, c2 = R5_C2, s2 = R5_S2;
        for (; c < m; ++c) {
            cpx a0=b0[c], a1=B1[c], a2=B2[c], a3=B3[c], a4=B4[c];
            ...
            cpx v1 = mulI(mkc(cre(t3)*s1+cre(t4)*s2, cim(t3)*s1+cim(t4)*s2));
            cpx v2 = mulI(mkc(cre(t3)*s2-cre(t4)*s1, cim(t3)*s2-cim(t4)*s1));
            b0[c]=_mm_add_pd(a0,_mm_add_pd(t1,t2));
            B1[c]=cmul(_mm_sub_pd(u1,v1),p1[c]);
            B2[c]=cmul(_mm_sub_pd(u2,v2),p2[c]);
            B3[c]=cmul(_mm_add_pd(u2,v2),p3[c]);
            B4[c]=cmul(_mm_add_pd(u1,v1),p4[c]);
        }
    }
    static void idit5StageR(cpx* b0, u32 m) {
        ...
        for (; c + 2 <= m; c += 2) {
            ...
            __m256d x1=cmulconjv(_mm256_loadu_pd((const double*)(B1+c)), _mm256_loadu_pd((const double*)(p1+c)));
            __m256d x2=cmulconjv(_mm256_loadu_pd((const double*)(B2+c)), _mm256_loadu_pd((const double*)(p2+c)));
            __m256d x3=cmulconjv(_mm256_loadu_pd((const double*)(B3+c)), _mm256_loadu_pd((const double*)(p3+c)));
            __m256d x4=cmulconjv(_mm256_loadu_pd((const double*)(B4+c)), _mm256_loadu_pd((const double*)(p4+c)));
        ...
        for (; c < m; ++c) {
            cpx y0=b0[c];
            cpx x1=cmulconj(B1[c],p1[c]), x2=cmulconj(B2[c],p2[c]);
            cpx x3=cmulconj(B3[c],p3[c]), x4=cmulconj(B4[c],p4[c]);
            ...
            cpx v1 = mulI(mkc(cre(t3)*s1+cre(t4)*s2, cim(t3)*s1+cim(t4)*s2));
            cpx v2 = mulI(mkc(cre(t3)*s2-cre(t4)*s1, cim(t3)*s2-cim(t4)*s1));
            b0[c]=_mm_add_pd(y0,_mm_add_pd(t1,t2));
            B1[c]=_mm_add_pd(u1,v1); B2[c]=_mm_add_pd(u2,v2);
            B3[c]=_mm_sub_pd(u2,v2); B4[c]=_mm_sub_pd(u1,v1);
        }
    }
