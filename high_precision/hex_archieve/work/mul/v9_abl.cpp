...
static void resize(u32 n) {  // n = 复数点数
    ...
    #ifdef TW_ABLATE
    for (u32 i = twlen; i != ((n >> 1) < 1024u ? (n>>1) : 1024u); ++i)
#else
    for (u32 i = twlen; i != (n >> 1); ++i)
#endif
        tw[(i) & TWMASK] = cmul(base[i & (halfSize - 1)], base[halfSize | (i >> halfLog)]);
    twlen = n >> 1;
}
...
static void difFlat(cpx* d, u32 n, u32 bb) {
    ...
    for (; bs >= 2; bc <<= 2, st = bs >> 1, bs >>= 2) {
        ...
        u32 j = 0;
        cpx* s = d;
        if (base == 0) { bf2FwdOne(s, q, tw[(1) & TWMASK]); j = 1, s += st; }
        for (; j != bc; ++j, s += st)
            bf2Fwd(s, q, tw[(base + j) & TWMASK], tw[(base2 + 2 * j) & TWMASK], tw[(base2 + 2 * j + 1) & TWMASK]);
    }
    if (bs == 1) {  // 层数为奇数时剩最低一层
        ...
        for (; j != bc; ++j, s += 2) bfFwd(s, 1, tw[(base + j) & TWMASK]);
    }
}
static void ditFlat(cpx* d, u32 n, u32 bb) {
    ...
    for (; (q << 2) <= n; q <<= 2, bcOut >>= 2) {
        ...
        u32 j = 0;
        cpx* s = d;
        if (base == 0) { bf2InvOne(s, q, tw[(1) & TWMASK]); j = 1, s += st; }
        for (; j != bcOut; ++j, s += st)
            bf2Inv(s, q, tw[(base + j) & TWMASK], tw[(base2 + 2 * j) & TWMASK], tw[(base2 + 2 * j + 1) & TWMASK]);
    }
    if ((q << 1) <= n) {  // 层数为奇数时剩最高一层 (bc == 1)
        const u32 base = bb;
        if (base == 0) bfPlain(d, q);
        else bfInv(d, q, tw[(base) & TWMASK]);
    }
}
static void difRec(cpx* d, u32 n, u32 bb) {
    if (n <= (1u << FFT_LEAF_LOG)) { difFlat(d, n, bb); return; }
    const u32 q = n >> 2, b4 = bb << 2;
    if (bb == 0) bf2FwdOne(d, q, tw[(1) & TWMASK]);
    else bf2Fwd(d, q, tw[(bb) & TWMASK], tw[(bb << 1) & TWMASK], tw[((bb << 1) | 1) & TWMASK]);
    difRec(d, q, b4);
    difRec(d + q, q, b4 | 1);
    difRec(d + 2 * q, q, b4 | 2);
    difRec(d + 3 * q, q, b4 | 3);
...
static void ditRec(cpx* d, u32 n, u32 bb) {
    ...
    ditRec(d + 2 * q, q, b4 | 2);
    ditRec(d + 3 * q, q, b4 | 3);
    if (bb == 0) bf2InvOne(d, q, tw[(1) & TWMASK]);
    else bf2Inv(d, q, tw[(bb) & TWMASK], tw[(bb << 1) & TWMASK], tw[((bb << 1) | 1) & TWMASK]);
}
...
static void pointwise(cpx* F, cpx* G, u32 n) {
    ...
    for (u32 bs = 2, be = 3; bs != n; bs <<= 1, be <<= 1) {
        ...
        for (; f != be; ++f, --b) {
            ...
            cpx t = (f & 1) ? _mm_xor_pd(tw[(f >> 1) & TWMASK], ngm) : tw[(f >> 1) & TWMASK];
