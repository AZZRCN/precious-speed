...
// 单级平表杠杆 (已落地, 常开): twfull[i]=twg(i), twg 改为无分支纯 load 省 1 复乘。
//   Ir 指标下 cache miss 不计, 故净降 Ir; 所有 resize 调用点 n 均为 2 幂 (掩码 n-1 恒有效), 取用无分支。
alignas(64) static __m128d twbase[1u << 12];
static u32 twHalfLog = 0, twHalfSize = 0, twHalfMask = 0, twN = 0;
// 单级平表 twiddle (TW 杠杆, 常开): twfull[i] == 旧 twg(i) 精确值, 取用 1 load 省 1 复乘。
// 所有 resize 调用点 n 均为 2 幂 (已核验 lm=2^j / 3*2^(j-2) / 5*2^(j-3) => m,ts 皆 2 幂),
// 故掩码 n-1 恒有效; twg 改为无分支纯 load。容量 2^22 覆盖 LMMAX(8M) 下最大 ts=4M。
alignas(64) static cpx twfull[1u << 22];   // 单级表 (64MiB 虚拟保留, 仅触及页驻留)
static u32 twfullMask = 0;                  // = n-1 (n 为 2 幂); twg 恒用此掩码无分支 load
...
static inline __m256d cmul256(__m256d a, __m256d b) {  // 2 复数/迭代, per-lane 等同 cmul
    return _mm256_fmaddsub_pd(_mm256_unpacklo_pd(a, a), b, _mm256_mul_pd(_mm256_unpackhi_pd(a, a), _mm256_permute_pd(b, 1)));
}
...
static void resize(u32 n) {  // n = 复数点数; 只建 16 KiB 的 base 表
    ...
    // 单级表: 恒建 (n 恒为 2 幂, 掩码 n-1 有效)。twfull[i] = 旧 twg(i) 精确值, 取用免复乘。
    // 向量化构建: 每迭代 2 复数 (cmul 对 256 位天然 per-lane), 砍半构建 Ir (构建仅付一次/尺寸, 已被变换摊销)。
    twfullMask = n - 1;
    u32 i = 0;
    for (; i + 2 <= n; i += 2) {
        const __m128d lo0 = twbase[i & twHalfMask], lo1 = twbase[(i + 1) & twHalfMask];
        const __m128d hi0 = twbase[twHalfSize | (i >> twHalfLog)], hi1 = twbase[twHalfSize | ((i + 1) >> twHalfLog)];
        const __m256d R = cmul256(_mm256_set_m128d(lo1, lo0), _mm256_set_m128d(hi1, hi0));
        twfull[i] = _mm256_castpd256_pd128(R);
        twfull[i + 1] = _mm256_extractf128_pd(R, 1);
    }
    for (; i < n; ++i)
        twfull[i] = cmul(twbase[i & twHalfMask], twbase[twHalfSize | (i >> twHalfLog)]);
}
// 单级查表: 无分支纯 load (掩码 n-1 有效, n 恒为 2 幂)。省旧两级方案的 2 load + 1 复乘。
static inline cpx twg(u32 i)  { return twfull[i & twfullMask]; }
...
static void difFlat(cpx* d, u32 n, u32 bb) {
    ...
    for (; bs >= 2; bc <<= 2, st = bs >> 1, bs >>= 2) {
        ...
        // 单级表下 twlo/twhi == twg (已是完整旋转因子), 外提高位分量纯属冗余 -> 统一走 twg。
        for (; j != bc; ++j, s += st) {
...
static int  g_cycbudget_log2 = 48;          // CYCB: cyclic 精度预算 2^X. 默认48=对齐已验证线性预算(可证明安全, cyclic 项数≈线性一半故严格更安全); 47=原保守; 49/50 仅 mid 例额外红利但越过可证明安全界
...
int main() {
    ...
    { const char* e = getenv("CYCB"); g_cycbudget_log2 = e ? atoi(e) : 48; }
