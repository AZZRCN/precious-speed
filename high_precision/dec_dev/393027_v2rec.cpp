...
// 单级平表 (v2rec: 仅加单级表, 保留 difFlat 外提分支): twfull[i]==旧两级 twg(i) 精确值, 取用 1 load 省 1 复乘。
// 容量 2^22 覆盖 LMMAX(8M) 下最大 ts=4M。n 恒为 2 幂 (掩码 n-1 有效)。
alignas(64) static cpx twfull[1u << 22];
static u32 twfullMask = 0;
static inline __m256d cmul256(__m256d a, __m256d b) {
...
static void resize(u32 n) {  // n = 复数点数; 只建 16 KiB 的 base 表
    ...
    // 单级表: 恒建 (n 恒为 2 幂, 掩码 n-1 有效)。twfull[i] = 旧 twg(i) 精确值。标量构建 (v2rec 求稳, 不向量化)。
    twfullMask = n - 1;
    for (u32 i = 0; i != n; ++i)
...
// (保留 twlo/twhi 供 difFlat 外提分支使用, 与单级表并存)
static inline cpx twg_twolevel(u32 i) {
