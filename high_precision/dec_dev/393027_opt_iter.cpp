...
struct DifNode { cpx* d; u32 n; u32 bb; };
// 显式栈迭代 radix-4 DIF: 与递归版等价 (相同 bb 序、相同叶子调用、相同旋转因子),
// 但消除每节点 C++ 函数调用的 call/ret 与 6 个 callee-saved 寄存器保存/恢复 (~12 指令/节点),
// 1M 点 FFT 有 ~349k 节点, 此项开销可观. 输出与递归逐字节一致 (构造即保证).
static void difRec(cpx* d, u32 n, u32 bb) {
    if (g_sr && bb == 0) { sr_dif(d, n, g_sr_rev); return; }
    if (n <= (1u << FFT_LEAF_LOG)) { difFlat(d, n, bb); return; }
    DifNode stk[256]; int sp = 0;
    stk[sp].d = d; stk[sp].n = n; stk[sp].bb = bb; sp++;
    while (sp > 0) {
        --sp;
        cpx* dd = stk[sp].d; u32 nn = stk[sp].n; u32 bbb = stk[sp].bb;
        if (nn <= (1u << FFT_LEAF_LOG)) { difFlat(dd, nn, bbb); continue; }
        const u32 q = nn >> 2, b4 = bbb << 2;
        if (bbb == 0) bf2FwdOne(dd, q, twg(1));
        else { const cpx w1 = twg(bbb << 1); bf2Fwd(dd, q, twg(bbb), w1, mulI(w1)); }
        // 子节点入栈 (LIFO: 最后压入者先处理, 与递归深度优先序一致; 处理序不影响结果)
        stk[sp].d = dd + 3 * q; stk[sp].n = q; stk[sp].bb = b4 | 3; sp++;
        stk[sp].d = dd + 2 * q; stk[sp].n = q; stk[sp].bb = b4 | 2; sp++;
        stk[sp].d = dd + q;     stk[sp].n = q; stk[sp].bb = b4 | 1; sp++;
        stk[sp].d = dd;         stk[sp].n = q; stk[sp].bb = b4;     sp++;
    }
}
...
struct DitNode { cpx* d; u32 n; u32 bb; bool done; };
// 显式栈迭代 radix-4 DIT (后序: 先子节点后合并蝶形), 与递归版等价, 消除每节点调用开销.
static void ditRec(cpx* d, u32 n, u32 bb) {
    if (n <= (1u << FFT_LEAF_LOG)) { ditFlat(d, n, bb); return; }
    DitNode stk[256]; int sp = 0;
    stk[sp].d = d; stk[sp].n = n; stk[sp].bb = bb; stk[sp].done = false; sp++;
    while (sp > 0) {
        --sp;
        cpx* dd = stk[sp].d; u32 nn = stk[sp].n; u32 bbb = stk[sp].bb; bool done = stk[sp].done;
        if (!done) {
            if (nn <= (1u << FFT_LEAF_LOG)) { ditFlat(dd, nn, bbb); continue; }
            const u32 q = nn >> 2, b4 = bbb << 2;
            // 重新压入本节点(done=true, 待子节点完成后合并) + 4 子节点 (LIFO, 子节点先处理)
            stk[sp].d = dd; stk[sp].n = nn; stk[sp].bb = bbb; stk[sp].done = true; sp++;
            stk[sp].d = dd + 3 * q; stk[sp].n = q; stk[sp].bb = b4 | 3; stk[sp].done = false; sp++;
            stk[sp].d = dd + 2 * q; stk[sp].n = q; stk[sp].bb = b4 | 2; stk[sp].done = false; sp++;
            stk[sp].d = dd + q;     stk[sp].n = q; stk[sp].bb = b4 | 1; stk[sp].done = false; sp++;
            stk[sp].d = dd;         stk[sp].n = q; stk[sp].bb = b4;     stk[sp].done = false; sp++;
            continue;
        }
        const u32 q = nn >> 2;
        if (bbb == 0) bf2InvOne(dd, q, twg(1));
        else { const cpx w1 = twg(bbb << 1); bf2Inv(dd, q, twg(bbb), w1, mulI(w1)); }
    }
}
