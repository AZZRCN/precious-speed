...
// ===== 缓存层级宏 (任务2: 按访问重要度贴合 cache 习惯) =====
// 延迟阶梯: L1(32KB,~1cy) << L2(512KB,~4cy) << L3(32MB/CCD,~12cy) << DRAM(~60cy)。
// 策略: 叶核留 L1/L2 (cache-oblivious 递归已做); FB/GB/FMG 活跃集压进 L3;
//       冷数据(IO 缓冲) 解析后不再访问, 由替换策略自然逐出, 不主动污染 L3。
// 提交 #393027 前: 把 L3_BYTES 改成 AMD EPYC 7B13 实测可用值 (单 CCD 标称 32MB,
//       但评测进程可能与其他核共享 CCD, 实际可用须 VM/真机实测 —— 见下方 L3 宏调试法)。
#ifndef L3_BYTES
#define L3_BYTES (32u << 20)   // 默认 32MB (VM Intel / 7B13 单 CCD 标称)
#endif
#ifndef L2_BYTES
#define L2_BYTES (512u << 10)  // Zen3 每核 L2 = 512 KiB
#endif
#ifndef L1_BYTES
#define L1_BYTES (32u << 10)   // Zen3 L1D = 32 KiB
#endif
// WORK 峰值 = 16*MAXC limbs (递归 wp 栈), 与 L3 无关 -> 按 MAXC 定上限, 省 ~16MB 冷足迹
static constexpr size_t WORK_CAP = 2000000;   // 16 MB (峰值 ~12.8MB, 留 3.2MB 余量)
// FB/GB(2*LMMAX*8) + FMG 活跃(2*FMCAP*8) + limbs(~10MB) 应 <= L3_BYTES。
// 默认 32MB 给满 1<<20; 显式设更小 L3 时按比例缩 (正确性由 mul 内 `L>LMMAX` 回退保证)。
static constexpr u32 LMMAX_D = (L3_BYTES >= (32u << 20)) ? (1u << 20) : (L3_BYTES / 32u);
static constexpr u32 FMCAP_D = (L3_BYTES >= (32u << 20)) ? (1u << 20) : (L3_BYTES / 32u);
#ifndef BZ_CUTOFF
...
alignas(HP) static u64 WORK[WORK_CAP];                   // Newton/BZ 工作栈 (压到峰值+余量, 省 L3 冷足迹)
static u64* wp = WORK;

static constexpr u32 LMMAX = LMMAX_D;
...
static constexpr u32 FMCAP = FMCAP_D;
...
// v-merge: FMG3 别名 FMG2 (Barrett 环形/线性路径互斥, 见 1821/1825 判定),
//   FMG4 别名 FMG1 (invertappr 的 cyc_prep 在 1818 早于 Barrett 循环 1820 完成, 时间不重叠)。
//   4 缓冲(32MB) -> 2 缓冲(16MB), 省 16MB L3 冷足迹; 活跃集 FMG1+FMG2 同时驻 L3。
#define FMG3 FMG2
#define FMG4 FMG1
