#pragma once
#include "prod_fft.h"     // 复用全局 g_sr / sr_dif / 类型别名; fft_ref 作为两级表参照
#include <cstdlib>
namespace fft_flat {
using cpx = __m128d;
// 单级大表变体: twflat[i] 预乘好 = cmul(base[i&mask], base[halfSize|(i>>halfLog)])
// -> twg(i) 退化为一次 load, 省掉每次 twiddle 的 1 复乘 + 1 load。
// 值与两级表逐位相同 (同一 cmul 预计算), 故输出可 bytecmp。
// 代价: 表 16*n 字节 (n=2^21 时 32 MiB, 出 L3) -> Ir 降但 miss 升。
alignas(64) static __m128d twbase[1u << 12];
static __m128d* twflat = nullptr;
static u32 twFlatCap = 0;
...
static void resize(u32 n) {  // n = 复数点数; 建 base 表后展开成 n 项大表
    if (n == twN) return;
    twN = n;
    ...
    // 实测(tw_idx): 最大 twiddle 索引恒为 n/2 - 1 -> 表只需 n/2 项 = 8n 字节
    const u32 cap = n >> 1;
    if (twFlatCap < cap) {
        if (twflat) std::free(twflat);
        twflat = (__m128d*)aligned_alloc(64, (size_t)cap * sizeof(__m128d));
        twFlatCap = cap;
    }
    // 逐位复刻两级表的乘积, 保证与 fft_ref::twg(i) 位相同
    for (u32 i = 0; i != cap; ++i) {
        const u32 hi = i >> twHalfLog;
        twflat[i] = (hi < twHalfSize)
            ? cmul(twbase[i & twHalfMask], twbase[twHalfSize | hi])
            : _mm_setzero_pd();     // 越界项: 若被查到, 比特级对照会立刻暴露
    }
}
#ifdef TW_TRACE
static u32 twMaxIdx = 0;
#define TW_HIT(i) do { if ((i) > twMaxIdx) twMaxIdx = (i); } while (0)
#else
#define TW_HIT(i) ((void)0)
#endif
// 单级查表: 1 次 load, 无复乘
static inline cpx twg(u32 i) { TW_HIT(i); return twflat[i]; }
// 大表下高位外提无意义: twlo 直接查表 (忽略 hi), twhi 返回占位
static inline cpx twlo(u32 i, cpx) { TW_HIT(i); return twflat[i]; }
